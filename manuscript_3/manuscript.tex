...
calculations and validated against Monte Carlo simulation.
\end{abstract}
...
Section~\ref{sec:constructions} puts all of this to work on constructions that have been solved
separately in the literature. Each is handled by writing down its chygraph
mapping and substituting, which is the point of the programme: the calculation
is done once, in Secs.~\ref{sec:map}--\ref{sec:joint}, and reused.
...
\section{Chygraphs and the threshold tensor}
...
The threshold calculation rests on four $L\times L$ matrices.
...
averages. From these one assembles the six-index tensor
\cite{vazquez2024comnet}
\begin{align}
...
pseudo-order parameter with a simpler algebraic form.

\section{The self-consistency map}\label{sec:map}
...
\begin{align}
  ...
  \label{eq:firstderiv}
\end{align}
so Eq.~\eqref{eq:A} uses exactly the first derivatives of these functions at
$1$. In this section I take the generating functions to factorise over target
...
The map is solved by iteration. $F$ carries $[0,1]^{2L^2}$ into itself and has
...
subcritical phase. Substituting it into Eq.~\eqref{eq:P} gives $S_l$.
...
it $\theta$, $\Lambda$ and the amplitude $B$ of Sec.~\ref{sec:amplitude}, while
$S$ at a given point in parameter space is evaluated numerically.
...
every mapping considered here: the Jacobian of
...
(Fig.~\ref{fig:degeneracy}). They also differ in the critical amplitude derived
below, $B=4/3$ against $B=8/9$, which is visible as the different initial slopes
...
$\nabla P^l\cdot r$ is a first directional derivative. And the $2L^2$ unknowns
reduce: discarding iteratively every index whose row or column of $J$ vanishes
...
are precisely the unknowns one would have written down by hand.

\subsection{Closed forms}\label{sec:closedforms}

Table~\ref{tab:amplitudes} collects $B\equiv B_0$ on the critical manifold
$\lambda=1$ for the standard mappings. Two results are worth isolating. For a
...
\begin{equation}
  ...
  \label{eq:Bmultiplex}
\end{equation}
verified symbolically for $L=1,2,3$ hyperedge types: each layer contributes to
...
alongside $\theta$ and $\Lambda$. Its non-negativity also settles, for anything
...
Fig.~\ref{fig:triangles}. They are further
cross-checked against an independent numerical evaluation of Eq.~\eqref{eq:B}
using numerical eigenvectors on the full, unreduced index set, confirming that
the core reduction is exact.
...
Eqs.~\eqref{eq:Qm}--\eqref{eq:Qp} when the generating functions factorise.
...
of the two complex layers, in the order the layers are listed.}
\begin{ruledtabular}
\begin{tabular}{llll}
...
follow from Secs.~\ref{sec:map}--\ref{sec:joint} unchanged.
...
through independent at $w=1/2$ to maximally positive at $w=1$. The site
