...
% The text block is 4.05in wide and stays that width: inner+outer is 0.95in
% whatever the split, so moving one margin moves the block bodily and never
% reflows a line. It began at inner=0.55/outer=0.4, which put the body over the
% gutter boundary a print check draws -- on a verso page the flag is at the
% right edge and on a recto at the left, both of them the inner edge. Shifted
% 0.10in toward the outside. Not further: microtype lets glyphs hang past the
% nominal block by up to 0.05in, and a print check measures ink rather than the
% block, so a nominal 0.25in outer leaves only 0.199in of real clearance at the
% TRIM edge -- which is the unforgiving one, since trim varies and the gutter
% merely tightens. At 0.30in the worst measured ink clearance is 0.249in, on
% the 0.25in floor, and the gutter takes everything left over.
\usepackage[paperwidth=5in,paperheight=8in,includehead,
            inner=0.65in,outer=0.30in,top=0.56in,bottom=0.64in]{geometry}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{graphicx}
