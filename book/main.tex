...
%
% It is here at 0.65/0.3 because a print check put the body over the gutter
% boundary: on a verso the flag lands at the right edge, on a recto at the
% left, and both of those are the inner edge. 0.10in of slide toward the
% outside is the most available, not a chosen round number -- microtype lets
% glyphs hang up to 0.05in past the nominal block and a print check measures
% ink, so nominal 0.30in outer already leaves only 0.249in of real clearance at
% the TRIM edge, which is the unforgiving side: trim varies per copy, while a
% tight gutter merely reads badly. Any further and the trim edge is the
% violation instead.
%
...
\usepackage[paperwidth=5in,paperheight=8in,includehead,
            inner=0.65in,outer=0.3in,top=0.56in,bottom=0.64in]{geometry}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{graphicx}
