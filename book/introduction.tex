\chapter{Statistical physics on networks}
\label{ch:introduction}

\setcounter{page}{1}
\pagenumbering{arabic}

Nothing in this chapter is new. Everything in it is used later, and the order is
the order the book will need it in. A reader who already knows what a
generating function does to a degree distribution, what the cavity method is,
and why replica symmetry sometimes fails, should skip to
Chapter~\ref{ch:chygraphs} --- except perhaps for the last section, which is
this chapter's own argument and the reason the rest of the book exists.

The argument is short enough to state now. Every method here works by pretending
the network is a tree. Real networks are not trees, and the specific way they
fail to be trees is that your friends know each other. That is the problem
Chapter~\ref{ch:chygraphs} is designed to solve.

\section{Networks, and what we put on them}
\label{sec:networks}
...
Two more quantities, both of which recur on every page of this book.

\paragraph{The excess degree.} Pick a link at random and walk along it. You
arrive at a node, and the question that matters for everything downstream is not
how many links that node has but how many \emph{others} --- how many ways there
are to keep going. Because a node with $k$ links is $k$ times more likely to be
found at the end of a randomly chosen link than a node with one, the degree you
arrive at is not drawn from $p(k)$ but from the size-biased distribution
...
$0.006$ to $0.65$. Hold on to this quantity. Section~\ref{sec:treebreaks} is
about it, and so, in the end, is the book.
...
Take a network and destroy it slowly. Keep each link with probability $p$ and
delete it otherwise. For small $p$ what remains is a scatter of small fragments.
...
The question is where. Here is the argument, and it is worth following in words
before any algebra, because it is the argument the whole book generalises.

Start at a node and follow a surviving link. You arrive somewhere, and from
there you can continue along $\bar k$ further links, of which a fraction $p$
...
\begin{equation}
  ...
  \label{eq:molloyreed}
\end{equation}
which is the Molloy--Reed criterion \citep{molloy1995}. For a Poisson network,
...
Three things about this argument deserve flagging, because all three carry
through to Chapter~\ref{ch:percolation} unchanged.
...
exact when the network has no short loops, and it is wrong when it does --- for
the simple reason that on a loop the exploration comes back to somewhere it has
already been, and a branching process has no way to know that.
...
once you are past it, which is the subject of Chapter~\ref{ch:giant}.

Third, deleting links at random and deleting nodes at random are the same
calculation with a different bookkeeping, not two different problems.
...
parameter are the linearisation and the full solution of one equation, a point
Chapter~\ref{ch:giant} makes a great deal of.
\end{calculation}
...
combinations of variables are cheap. Three such models run through the book, and
it is worth meeting all three now, on the same small graph, because their family
...
are the same branching process counted with a different weight. That is not a
coincidence and it is not a metaphor; Chapter~\ref{ch:potts} shows the two are
literally the same model at two values of one parameter.

\subsection{Vertex cover}
\label{sec:cover-intro}

Now a problem with no temperature in it at all. A \emph{vertex cover} of a graph
...
Figure~\ref{fig:threemodels} puts the three side by side on one graph, which
is the point --- they differ in what decorates the nodes and what is paid for,
not in the object underneath.
...
There is one caveat about that limit, and this book is largely about it. Taking
...
This is wrong, and the way it is wrong is instructive. Compare it with
Eq.~\eqref{eq:tc-intro}: mean field gives $\beta J\ave{k}=1$ where the correct
...
$\ave{\bar k}$ --- it forgets that you arrived somewhere and cannot leave the
way you came --- and it uses $\beta J$ where it should use $\tanh\beta J$,
...
Here is the idea that the whole book rests on, and it is a geometric idea before
it is a calculation.
...
on in another. That independence is the whole trick, and it converts a problem
...
statisticians and computer scientists independently.
...
Three features of this calculation are worth naming, because they are exactly
what Chapter~\ref{ch:statmech} generalises. The message is a \emph{number} here
...
\section{Replicas, and when the answer is wrong}
\label{sec:replicas}
...
has assumed that the system has essentially \emph{one} state --- that if you ran
the model twice, the two runs would look statistically alike. This is
...
graph it happens at mean degree $e$, the very point where leaf removal stalls
\citep{weigt2000,bauer2001}, which is not a coincidence.
...
of distributions \citep{mezard2001}. This book does not do that calculation. It
does, however, need to know when the calculation it \emph{does} do has stopped
being right, and there are three reliable signals.
...
squaring. That squaring is the whole difference between the ferromagnetic
...
None of these tells you the right answer. All of them tell you that the answer
in hand is wrong, which is worth a great deal, and this book flags every place
they fire.

\section{The tree assumption, and why real networks break it}
\label{sec:treebreaks}

Now the argument this chapter exists to make.

Look back over what has been done. The percolation threshold came from a
...
away from a node and you must not come back.
...
by Monte Carlo at $14.2$ per cent. The sign is worth registering in advance,
because it is not the one most people guess: clustering \emph{lowers} $T_{c}$.
...
That second example is the one to keep in mind, because it rules out the obvious
response. One cannot rescue the situation by measuring the degree distribution
...
So the field has, and has had for twenty-five years, a beautiful method and a
systematic reason why it should not apply to the objects it is applied to. The
next chapter's response is not to patch the method. It is to change what the
