...
This chapter assembles that machinery: what a generating function does to a
...
$0.006$ to $0.65$. Every method in this chapter assumes $C=0$, and
Sec.~\ref{sec:treebreaks} measures how far real networks are from that
assumption.
...
Figure~\ref{fig:basics} shows the three quantities.
...
the observation Chapter~\ref{ch:giant} is built on. Chapter~\ref{ch:percolation} writes both lines for a chygraph, Eqs.~\eqref{eq:Qminus} and~\eqref{eq:Sl}, and \code{giant.\allowbreak Chygraph.\allowbreak apply} and \code{giant.\allowbreak Chygraph.\allowbreak node\_\allowbreak fraction} are what evaluate them.
...
Figure~\ref{fig:branching} draws the argument: one branch begetting more than
one branch, and why it is exact on a tree.

\section{Statistical mechanics}
\label{sec:models}
\index{partition function|textbf}
\index{Boltzmann measure|textbf}
\index{free energy}

Percolation asks about the network itself. The rest of the book puts something
\emph{on} the network --- a variable $\sigma_{i}$ at every node, and an energy
$\mathcal{H}(\sigma)$ that says which combinations of variables are cheap. One
sum over configurations then carries everything:
\begin{equation}
  Z=\sum_{\{\sigma\}}
    e^{-\beta\bigl[\mathcal{H}(\sigma)-B\sum_{i}\sigma_{i}\bigr]},
  \qquad
  \beta=1/T,
  \label{eq:Z-intro}
\end{equation}
the \emph{partition function}, with $\beta$ the inverse temperature and $B$ the
parameter conjugate to $\sum_{i}\sigma_{i}$: an external field where the
variables are spins, and a chemical potential where they count the members of a
set, which Chapters~\ref{ch:hittingset} and~\ref{ch:cover} write $\mu$ and
charge for rather than reward. It is held beside $\mathcal{H}$ rather than
inside it because it is the parameter Eq.~\eqref{eq:lnZ-intro} differentiates;
a model may equally write it in, as Eq.~\eqref{eq:ising-intro} does. Each
configuration carries the weight in the exponent and $Z$ normalises those
weights, so a configuration's share of $Z$ is its probability --- the Boltzmann
measure.

Equation~\eqref{eq:Z-intro} is a generating function, in the sense
Sec.~\ref{sec:percolation-intro} used for $G_{0}$: a weighted sum over the
objects of interest, arranged so that what one wants of them comes out by
differentiating rather than by counting. What it generates is decided by what
stands beside $\mathcal{H}$ in the exponent. Differentiating in the field,
\begin{equation}
  ...
  \label{eq:lnZ-intro}
\end{equation}
the mean variable per node and its response to the field: one derivative, and
then a second. The logarithm and not $Z$ itself, because $\ln Z$ grows with the
number of nodes, so its derivatives are quantities per node; $f=-\ln Z/\beta N$
is the \emph{free energy}, which Chapter~\ref{ch:statmech} computes for a
chygraph and then differentiates.

Five of the models that run through the book are introduced next, and all five
appear on the same small graph, so the family resemblance is visible. What
changes from one to the next is $\mathcal{H}$ and nothing else. For the wider
map --- spin models, $k$-core percolation, epidemic
...
preference and $B$ an external field pushing every spin one way. It is the first
thing to put into Eq.~\eqref{eq:Z-intro}, with the field written into
$\mathcal{H}$ rather than held beside it; the $B$ here and the $B$ there are the
same one.
...
\begin{equation}
  ...
  \prod_{\langle ij\rangle}\bigl[1-(1-x_{i})(1-x_{j})\bigr]=1,
  \label{eq:vc-intro}
\end{equation}
the bracket vanishing on a link with neither endpoint covered, so the product is
one exactly when every link is watched.
...
was the move that opened the subject \citep{weigt2000}. It also has a rule that
solves it outright wherever the graph is thin enough, which is
Sec.~\ref{sec:leafremoval-intro}.
...
The Hamiltonian is Eq.~\eqref{eq:vc-intro} with the constraint read over sets
rather than links:
\begin{equation}
  \mathcal{H}=\sum_{i}x_{i},
  \qquad\text{subject to}\quad
  \prod_{a}\Bigl[1-\prod_{i\in a}(1-x_{i})\Bigr]=1,
  \label{eq:hs-intro}
\end{equation}
the inner product running over as many members as the set $a$ has, and the
bracket vanishing on a set none of whose members is taken.
...
\label{sec:colouring-intro}
\index{graph colouring|textbf}

The last two models drop the minimisation and keep only the constraint.
Colouring asks for an assignment of $q$ labels to the nodes with no link joining
two that are alike. Nothing here is made small and no ground state is sought:
such an assignment is available or it is not, and what one wants to know is how
many links a graph can carry before it stops being available.

It is still a model of the same shape. Counting monochromatic edges,
\begin{equation}
  \mathcal{H}=\sum_{\langle ij\rangle}\delta_{\sigma_{i}\sigma_{j}},
  \qquad \sigma_{i}\in\{1,\dots,q\},
  \label{eq:col-intro}
\end{equation}
a proper colouring is a configuration of zero energy. Equation~\eqref{eq:col-intro}
is the \emph{antiferromagnetic} $q$-state Potts model --- Eq.~\eqref{eq:ising-intro}
with more than two states and the sign of the coupling reversed --- taken to zero
temperature. Deciding colourability is NP-complete for $q\ge3$ \citep{karp1972}.

On a random graph the colourings run out at a threshold that depends on $q$, and
locating it is harder than anything above: the replica-symmetric calculation
gives a definite number and that number is not the threshold.
Chapter~\ref{ch:colouring} computes both lines and says by how much they differ.

\subsection{Satisfiability}
\label{sec:sat-intro}
\index{satisfiability|textbf}

Satisfiability puts a group where colouring puts a link. Each of $M$
\emph{clauses} names $k$ of the $N$ Boolean variables and rules out a single one
of the $2^{k}$ ways of setting them; which one is fixed by the clause, through
the signs with which it names its variables. The density of clauses per
variable, $\alpha=M/N$, plays the part that the density of links plays for
colouring: below some $\alpha$ a random formula can be satisfied and above it
cannot \citep{mezard2002science,montanari2008}.

It is the same shape of model once more. A clause is a disjunction of $k$
\emph{literals}, a literal being a variable or its negation, and a formula is a
conjunction of clauses --- an \textsc{and} of \textsc{or}s:
\begin{equation}
  \begin{aligned}
    F&=\bigwedge_{a=1}^{M}C_{a},
      \qquad C_{a}=\bigvee_{i\in a}\ell_{i},\\
    \mathcal{H}&=\sum_{a=1}^{M}\bigl(1-C_{a}\bigr).
  \end{aligned}
  \label{eq:sat-intro}
\end{equation}
With $C_{a}\in\{0,1\}$ the truth value of clause $a$, $\mathcal{H}$ counts the
clauses left false, and a satisfying assignment is a configuration of zero
energy, exactly as a proper colouring is one of Eq.~\eqref{eq:col-intro}. A
clause is false only when all $k$ of its literals are, which is the one
assignment it rules out.

Colouring and satisfiability differ in one thing, and Chapter~\ref{ch:satisfiability}
turns on it. A monochromatic edge is forbidden whatever the colour, so colouring
is symmetric under permuting the $q$ states. A clause forbids a
\emph{particular} assignment, the one its signs pick out, and no such symmetry
survives. That is why colouring has a closed-form threshold in this book and
satisfiability does not.
...
\begin{scope}[yshift=-4.5cm]
  \node[tb,anchor=west] at (-0.3,2.85) {(d) colouring};
  \node[nd] (d1) at (0.15,1.55) {\tiny$1$}; \node[nd] (d2) at (1.25,2.05) {\tiny$2$};
  \node[nd] (d3) at (1.15,0.85) {\tiny$3$}; \node[nd] (d4) at (2.35,1.55) {\tiny$3$};
  \node[nd] (d5) at (2.25,0.35) {\tiny$1$};
  \draw[lnk] (d1)--(d2) (d1)--(d3) (d2)--(d3) (d2)--(d4) (d3)--(d5) (d4)--(d5);
  \node[lb,anchor=west,align=left] at (-0.3,-0.30)
    {one of $q$ colours at every node;\\ neighbours must differ};
\end{scope}
% ---- satisfiability
\begin{scope}[yshift=-4.5cm,xshift=5.0cm]
  \node[tb,anchor=west] at (-0.3,2.85) {(e) satisfiability};
  \node[nd] (e1) at (0.15,2.15) {\tiny$\mathsf{T}$};
  \node[nd] (e2) at (0.15,1.05) {\tiny$\mathsf{F}$};
  \node[nd] (e3) at (1.30,1.60) {\tiny$\mathsf{T}$};
  \node[nd] (e4) at (2.40,1.60) {\tiny$\mathsf{F}$};
  \node[nd] (e5) at (3.35,0.85) {\tiny$\mathsf{T}$};
  \begin{scope}[on background layer]
    \node[cx,fill=black!26,fill opacity=0.7,fit=(e1)(e2)(e3)]{};
    \node[cx,fill=black!26,fill opacity=0.7,fit=(e3)(e4)(e5)]{};
  \end{scope}
  \node[lb,anchor=west,align=left] at (-0.3,-0.30)
    {a value at every node; each group\\ rules out \emph{one} of its patterns};
\end{scope}
\end{tikzpicture}
\end{adjustbox}
\caption{\textbf{Five models, one object.} (a) The Ising model decorates the
...
boxes, are already the complexes of Chapter~\ref{ch:chygraphs}. (d) Colouring
keeps the links and drops the minimisation: $q$ states per node, and no link may
have its two ends agree. (e) Satisfiability keeps the groups and forbids one
pattern in each --- a clause is violated only when every one of its literals is
false. What the five share is a variable per node, an energy, and a question
about the resulting Boltzmann measure; what they differ in is what the message
of Sec.~\ref{sec:cavity} has to carry, and whether the question is how small a
solution can be or whether one exists at all.}
\label{fig:models}
\end{figure}

Figure~\ref{fig:models} puts the five side by side: they differ in what
decorates the nodes and what is paid for, not in the object underneath.

\subsection{What these have in common}

All five are a variable at every node, an energy summing local terms, and the
Boltzmann measure of Eq.~\eqref{eq:Z-intro}. Optimisation is the
...
They divide once, and it is worth naming the division because it decides what a
threshold means. The Ising model is thermal, and its transition is a change of
state. Vertex cover and hitting set are optimisations, and their question is how
small a solution can be. Colouring and satisfiability are neither: nothing is
minimised, the constraints are hard, and the question is whether a solution
exists at all. For those two the threshold is a density at which solutions run
out, which is a different object from a critical temperature and is located a
different way, which Chapters~\ref{ch:colouring}
and~\ref{ch:satisfiability} carry out.
...
\label{sec:leafremoval-intro}
\index{leaf removal|textbf}
\index{core|textbf}
\index{core fraction|textbf}

At low temperature, and where the local structure is simple enough, the optimal
value of a variable can sometimes be read off its immediate neighbourhood
without solving the model at all. Such a variable can be assigned that value
and deleted, together with whatever else the assignment settles, which leaves a
smaller instance of the same problem. Iterating this reduces the instance
before any approximation is applied, and the methods of the following sections
are then applied to whatever is left.

Vertex cover shows the rule in its plainest form. Consider a node of degree one
--- a \emph{leaf}. Its single link must be covered, and it can be covered at
either end. Covering it at the leaf can never be better than covering it at the
neighbour, because the leaf touches nothing else while the neighbour may touch a
great deal: take any cover that contains the leaf, swap the leaf for the
neighbour, and the result is a cover no larger than the one started from, since
the one link the leaf was covering is covered by the neighbour as well. So there
is always a minimum cover that takes the neighbour and leaves the leaf, and the
two can be settled and struck out without looking at the rest of the graph.

\begin{model}{Leaf removal for vertex cover}
\begin{rules}
\rl{1}{Find} Look for a node of degree one.
\rl{2}{Decide} Put its neighbour into the cover; leave the leaf out.
\rl{3}{Delete} Remove the leaf, the neighbour, and every link the neighbour
touched --- all of those links are now covered.
\rl{4}{Repeat} Return to rule~1 on what is left, and stop when no node of degree
one remains.
\end{rules}
\end{model}

This is \emph{leaf removal} \citep{bauer2001}, and what the rules produce is one
of exactly two outcomes. When they consume the whole graph, the cover assembled
along the way is a minimum cover, and the exchange argument above, applied once
per step, is a proof that it is minimum. When they stall --- when what is left
has no node of degree one anywhere in it --- that residue is called the
...
None of that was specific to vertex cover. Every model of
Sec.~\ref{sec:models} has variables its surroundings can decide, and the
procedure above applies wherever they do; only what counts as decided, and what
the deletion takes with it, change from model to model. Run to a fixed point it
either empties the object or stalls, and what it stalls on is the core. Three
chapters use it, on three models and three kinds of object.

It is an algorithm at zero temperature, and as a solver it is approximate. Zero
temperature, because forcing is a statement about ground states: at any $T>0$ a
variable merely leans, and leaning is not a reason to delete it. Approximate,
because emptying the object proves the answer it built while stalling proves
nothing --- a core is only where local forcing ran out, and what it costs still
has to be computed by the methods that follow. The caveat that closes
Sec.~\ref{sec:models}, about taking $T\to0$ inside an approximation, is a
caveat about this rule too.

The fraction of nodes that cannot be removed is the \emph{core fraction}, and
it will be denoted by
\[
  P_{C}(\text{model},\text{structure}).
\]
Both arguments are needed. The \emph{model} fixes what counts as forced, and different
models consume different amounts: a covering constraint on a link forces the
neighbour into the cover and takes it out of the graph, while an Ising coupling
determines an atom and eliminates nothing else. The \emph{structure} is what
the rule runs on --- $\mathrm{G}$ a graph, $\mathrm{H}$ a hypergraph, $\chi$ a
chygraph, and $\chi_{m}$ the merged chygraph of
Chapter~\ref{ch:metacomplex}.

So $P_{C}(\mathrm{VC},\mathrm{G})$ is the core just described, zero below
$\ave{k}=e$ on a Poisson graph and positive above it.
$P_{C}(\mathrm{VC},\chi)$ is the same rule on a chygraph, and
Chapter~\ref{ch:cover} finds it has no core-free phase at all once a complex
holds three members --- the threshold does not move, it ceases to exist.
$P_{C}(\mathrm{Ising},\chi)$ is the milder rule Part~IV runs, and it is a
different number on the same object: a ring of triangles glued at single atoms
gives $0$ for the covering rule, which takes the complex around a forced atom,
and $1/2$ for the coupling, which leaves it. That is why the model is an
argument and not a remark.
...
The cavity method rests on a property of trees, and the calculation follows
from it.
...
on in another. That independence is what the method uses: a problem in which
every variable is coupled to every other becomes a problem about one branch at
a time.
...
Figure~\ref{fig:cavity} draws it: the removed node, the branches its removal
separates, and the message each of them sends.
...
\paragraph{What belief propagation (BP) returns.} The messages are not themselves
the answer. Once they have stopped changing, the \emph{belief} at a node --- its
marginal, the probability that it takes each of its values --- is what the
messages arriving there multiply out to, and the belief on a link is the same
statement for the pair at its two ends. On a tree both are exact.

So is the free energy, and how that one is assembled is the part
Sec.~\ref{sec:gbp-intro} generalises. A tree is exhausted by its links and its
nodes, so $\ln Z$ is one term per link plus one per node, the node terms
discounted because a node of degree $k$ has already been counted once inside
each of the $k$ links that hold it:
\begin{equation}
  \ln Z=\sum_{\langle ij\rangle}\ln Z_{ij}
        +\sum_{i}\bigl(1-k_{i}\bigr)\ln Z_{i},
  \label{eq:bethe-intro}
\end{equation}
with $Z_{ij}$ and $Z_{i}$ the local partition functions the messages build. This
is the \emph{Bethe free energy}. What makes it right on a tree is that every
variable and every interaction is counted exactly once in it, and that ---
rather than the message update --- is the property Sec.~\ref{sec:gbp-intro}
keeps when it goes past trees. Chapter~\ref{ch:statmech} writes
Eq.~\eqref{eq:bethe-intro} for a chygraph.
...
book. Chapter~\ref{ch:statmech} does the separating: the interior sum is
\code{Chygraph.\allowbreak emitted\_\allowbreak field} and the factor that
appears here as $t$ is \code{Chygraph.\allowbreak u\_\allowbreak prime}, both
over the module \code{cavity}.
...
The copies are worth a paragraph, because the derivation in
Sec.~\ref{sec:replicas-chy} rests on them. What has to be averaged over the
disorder is $\ln Z$, and there is no good way to average a logarithm. What can
be averaged is a power, $\overline{Z^{n}}$, because for integer $n$ it is just
$n$ independent copies --- \emph{replicas} --- of the same system, and the
average factorises over the objects the disorder lives on. The two are joined by
\begin{equation}
  \overline{\ln Z}=\lim_{n\to0}\frac{\overline{Z^{n}}-1}{n},
  \label{eq:replicatrick}
\end{equation}
which is the \emph{replica trick}. The step that is not justified is the last
one. $\overline{Z^{n}}$ has been computed at the positive integers and the limit
is taken at zero, so a function known on the integers is being continued to the
real line, and nothing in the derivation shows that the continuation is unique
or that the limit and the thermodynamic limit commute. It is a non-rigorous but
very well tested method: its predictions have been confirmed case by case for
half a century, and proved, for a few models, only much later. This book uses it
once, in Sec.~\ref{sec:replicas-chy}, and says so there.
...
This section collects the tree assumption made by every method above and
measures how badly real networks violate it, since that violation is what the
...
the same statement: \textbf{the network must be locally treelike}. Following
links away from a node must not lead back to it.
...
Supplying a description that does contain it is the subject of
Chapter~\ref{ch:chygraphs} and of everything after it.
...
Equation~\eqref{eq:bethe-intro} is exact on a tree because every variable and
every interaction in it is counted once. When the network is not a tree that
counting fails. The remedy, due to Kikuchi and given its message-passing form by
\citet{yedidia2005}, is to keep the counting and enlarge the units: work with \emph{regions} --- sets of
...
sub-regions rather than from node to node. This is \emph{generalised belief
propagation}, $\mathrm{GBP}$ for short.
...
Eq.~\eqref{eq:mobius-intro} returns $c_{v}=1-k$, which is
Eq.~\eqref{eq:bethe-intro}, and the update reduces to
...
It is also not the only repair in print. \citet{cantwell2019} attack the same
difficulty from the other end. Instead of enlarging the units the messages live
on, they enlarge the \emph{neighbourhood} each message is conditioned on, and
write equations that are exact for the loops falling within a given radius, the
radius being paid for in enumeration. Set beside Kikuchi's, the two make the
same trade in different currency: exactness bought by summing exactly over a
larger local object, a region in one case and a neighbourhood in the other. That
is worth noticing before the next chapter, because the next chapter makes a
third choice of local object --- a complex --- and everything after it is what
