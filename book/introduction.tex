...
The book measures what is left often enough to need a symbol for it. Write
\[
  P_{C}(\text{model},\text{structure})
\]
for the \emph{core fraction}, the share of atoms the rule cannot strip. Both
arguments do work. The \emph{model} fixes what counts as forced, and different
...
The cavity method is geometric before it is a calculation.
...
That is the gap this book is written to close: the method works, and the
property of real networks that says it should not is structural rather than a
matter of measuring $p(k)$ more carefully.
