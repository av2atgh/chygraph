...
Chapter~\ref{ch:hittingset}. The rule itself is more general than the problem
it is stated on, and Sec.~\ref{sec:leafremoval-intro} takes it out.
...
\subsection{Leaf removal, and the core}
\label{sec:leafremoval-intro}
\index{leaf removal}
\index{core}
\index{core fraction|textbf}

Section~\ref{sec:cover-intro} deleted a leaf and its neighbour and repeated.
That rule is not a fact about vertex cover. Each model above has variables its
surroundings decide for it, and where a variable is decided there is nothing
left to compute: fix it, delete it, delete whatever fixing it consumes, and put
the question again to what remains. Run to a fixed point, the procedure either
empties the object or stalls, and what it stalls on is the \emph{core}. Three
...
has to be computed by the methods of the rest of this chapter. The caveat above
about taking $T\to0$ inside an approximation is a caveat about this rule too.
...
So $P_{C}(\mathrm{VC},\mathrm{G})$ is Sec.~\ref{sec:cover-intro}'s core, zero
below $\ave{k}=e$ on a Poisson graph and positive above it.
