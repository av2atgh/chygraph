...
\paragraph{A hypergraph} is one layer of cardinality $c$. The division is the
same one and gives $\ave{k}(c-1)=e$: the instability does not move at all, at
...
\paragraph{A clustered graph} is a link layer at $c=2$ beside a clique layer at
$c=3$. Two cardinalities leave the two sums of Eq.~\eqref{eq:kcmulti} out of
proportion, so there is no division to make and no closed form: the pair is
solved, and the answer depends on how the degree is split between the layers.
...
Here we do not work on the one-step replica-symmetry-breaking calculation.
