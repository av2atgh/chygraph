\chapter{The hitting set problem}
\label{ch:hittingset}
...
The moral is the chapter's own version of a theme the book keeps returning to.
...
and differ in one line of the interior sum; the family is worth keeping in view,
because the failure this chapter is about is invisible at the end where everyone
works.
...
Here is the standard move. As $\mu\to\infty$ the cavity fields diverge, so
rescale them: put $h=\mu z$ and keep only the integer $z$. The up step of
...
its other members is free to stay out, and a node counts its warnings. With
$\sigma_{l}$ the probability that a node reached through a layer-$l$ complex has
$z=1$,
\begin{equation}
...
\begin{calculation}{the hard-field threshold}
...
The mechanism is worth naming precisely, because ``the ansatz is too coarse''
would be too vague. Inside such a hyperedge every vertex sits at $z=0$: the
...
Ref.~\citep{mezard2007} finds a genuine replica-symmetric region --- though it
...
\begin{align}
  ...
  \label{eq:soft}
\end{align}
with the density read off the full field, $\rho=\sigma(-\mu+\sum_{b\ni i}
v_{b\to i})$.
...
\begin{calculation}{the regular solution, and what the integer ansatz cannot hold}
...
\begin{equation}
  ...
  \label{eq:hRS}
\end{equation}
Stop and look at that. The cavity field carries a \emph{fraction} $1/L$ of
$\mu$. An ansatz restricted to integer multiples of $\mu$ cannot represent it,
and that --- not any subtlety about correlations --- is the whole of why
...
none is wasted, so the counting bound is saturated exactly.
\end{calculation}

Equation~\eqref{eq:hRS} and $\rho=1/K$ are the closed forms of
Ref.~\citep{mezard2007}, and population dynamics reproduces both --- the field
...
Ref.~\citep{mezard2007} (star), $0.1667<0.178<0.2522$. Generated by
\code{figs/hittingset.py}.}
\label{fig:hitting}
\end{figure}
...
more members --- $56\%$ for $c=4$ alone, and under $1\%$ once ordinary edges
dominate. That last number is the reassuring one: it says the twenty-five years
...
Two effects are worth recording, and they point in opposite directions.
...
\paragraph{A confound worth naming.} The obvious positive-correlation
...
such comparison. In the family above it rises from $0.033$ to $0.47$ as the
correlation is pushed to its extreme, so only the small-spread end of the
positive branch is a clean measurement. The negative branch keeps both classes
populated throughout and is clean everywhere.
...
\begin{equation}
  ...
  \label{eq:entropy}
\end{equation}
which is Ref.~\citep{mezard2007}'s expression, reproduced by
...
What one-step replica symmetry breaking would change, this book does not
compute. What it can say is where the replica-symmetric answer stops being
trustworthy, and on which side of the truth it then falls.

\section{Checks}
\label{sec:hs-checks}

\emph{Against known results.} Equation~\eqref{eq:kc} is verified to eleven
digits for $c=2,\dots,20$, and returns $e$ at $c=2$. At cardinality two both
treatments return the Weigt--Hartmann cover size, the hard field to six digits.
Equations~\eqref{eq:hRS} and~\eqref{eq:entropy} reproduce
Ref.~\citep{mezard2007}'s closed forms, the first to five decimals by population
dynamics and the second to nine digits by Eq.~\eqref{eq:sgen}.

\emph{Against arithmetic.} Section~\ref{sec:hardfail}'s counterexample is not a
numerical check but a proof: the answer is $1/3$ by counting, the soft field
returns $0.3333$, and the hard field returns $0.5$. Nothing about it depends on
an implementation being right.

\emph{Where the numbers are soft.} Soft-field densities on Poisson ensembles
come from a population of finite size and carry a scatter of a few parts in a
thousand that does not shrink cleanly with the population. They are quoted to
three decimals and the spread across seeds is reported with them. The regular
cases carry no such scatter, for the reason given above. Both facts are
properties of the estimator rather than of the theory, and the distinction is
Sec.~\ref{sec:assumes}'s.
