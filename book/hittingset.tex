...
\paragraph{A graph} is one Poisson layer of links at chy-degree two, and is
where the two problems coincide. At cardinality two,
...
\paragraph{A hypergraph} is one Poisson layer of $3$-hyperedges at chy-degree
one, which is the same two neighbours per node as the row above. It is the
structure this chapter was written for, and
...
\paragraph{A multiplex network} is two Poisson link layers at chy-degree one
each, and is a graph as far as this chapter can tell. Every layer is a link
layer, hitting a link is covering it, and what is being
solved is vertex cover of the union of the layers. Nothing in
Sec.~\ref{sec:hardfail} applies, the hard field is exact, and
Eq.~\eqref{eq:kcmulti} puts the point at total chy-degree $e$ however many link
types there are and however the degree is split between them. That is
Sec.~\ref{sec:fivecover}'s conclusion reached by the other route and for the
same reason, which is that the Jacobian has rank one.

\paragraph{Interacting graphs and hypergraphs} is three layers: three link
layers at chy-degree $2/3$ for the graph reading, three layers of
$3$-hyperedges at chy-degree $1/3$ for the hypergraph one. It is where this
chapter's two partitions visibly come apart, and the only structure that shows
it. For
...
\paragraph{A clustered graph} is a Poisson link layer at chy-degree one beside
a Poisson triangle layer at chy-degree one half. It is the only one of the five
whose layer list is mixed by construction --- links at cardinality two,
triangles at three --- and so the only row whose instability is not $e$. It sits at $3.28$ neighbours per node
