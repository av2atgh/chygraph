...
A committee has to contain at least one member of every department. A set of
cameras has to watch every corridor. A test suite has to touch every branch of
the code. In each case a family of sets is handed over and the task is to choose
as few elements as possible while leaving no set in the family untouched. That
is the \emph{hitting set} problem, one of the twenty-one Karp put on his
original list \citep{karp1972}, and it is the natural higher-order relative of
vertex cover: the sets are the complexes, and a set of two elements is an edge.

It is also, for this book, the place where something breaks.
...
and Sec.~\ref{sec:soft} gives the repair. Section~\ref{sec:hsfive} then runs
both treatments over the five structures of Fig.~\ref{fig:mappings}, one
paragraph each, and finds that they do not split the list the same way
Chapter~\ref{ch:cover} does.
...
The bracket finds the solution and diagnoses it in one pass, which is more
informative than watching an iteration fail to converge.
...
\paragraph{Over several layers.} Equation~\eqref{eq:kc} is the one-layer form,
and three of Fig.~\ref{fig:mappings}'s five structures carry more than one
layer. The derivation goes through with one change. With Poisson layers every
$\bar\Phi^{(m)}$ is $\Phi$, so $\sigma$ is common to the layers; write
$y_{l}=\kappa_{l}\sigma^{\,c_{l}-1}$ and $Y=\sum_{l}y_{l}$, so that the fixed
point is $\sigma=e^{-Y}$. The Jacobian
$J_{ml}=-(c_{l}-1)\sigma^{\,c_{l}-2}\kappa_{l}\sigma$ does not depend on $m$,
so it has rank one and its one non-zero eigenvalue is its trace. The pair is
then
\begin{equation}
  \sum_{l}\kappa_{l}e^{-(c_{l}-1)Y}=Y,
  \qquad
  \sum_{l}(c_{l}-1)\,\kappa_{l}e^{-(c_{l}-1)Y}=1.
  \label{eq:kcmulti}
\end{equation}

Divide the second by the first. If every $c_{l}$ is the same $c$ the two sums
differ by exactly that factor, so $(c-1)Y=1$, and substituting back gives
$\bigl(\sum_{l}\kappa_{l}\bigr)(c-1)=e$: Eq.~\eqref{eq:kc} again, with the
\emph{total} chy-degree, at any number of layers. A spread in cardinality is
precisely what leaves nothing to divide. That is the mechanism behind the first
of Sec.~\ref{sec:hsstructures}'s two effects, which is reported there as three
numbers, and it says the effect is a property of the spread and not of the
cardinality: one layer of hyperedges of size twenty sits at the same $e$
neighbours per node as a graph does.
...
structures of Fig.~\ref{fig:mappings}. This is that list for hitting set, and
Sec.~\ref{sec:fivecover} is the same list for vertex cover. It is worth doing
twice, because the list does not split the same way both times.

Chapter~\ref{ch:cover} has one partition and it falls at cardinality two: below
it leaf removal certifies a cover, above it there is no core-free branch to
certify with. This chapter has \emph{two}, and they do not coincide. The
hard-field limit is exact at cardinality two and wrong above it, which is
Sec.~\ref{sec:hardfail} and is the same line. The instability,
Eq.~\eqref{eq:kcmulti}, is moved by a \emph{spread} in cardinality and not by
cardinality itself, which is a different line and cuts the five somewhere else.
Table~\ref{tab:fivehs} carries both columns for that reason.

\begin{table}[t]
\centering\small
\begin{adjustbox}{max width=\linewidth}
\input{figs/tab-five-structures-hs}
\end{adjustbox}
\caption{\textbf{The two treatments on the structures of
Fig.~\ref{fig:mappings}.} All six rows at one density of constraints, two
neighbours per node: a layer-$l$ complex supplies $c_{l}-1$ of them per member,
so $\sum_{l}\kappa_{l}(c_{l}-1)$ is held fixed and split evenly across the
layers, Poisson in each. \emph{Instability} is Eq.~\eqref{eq:kcmulti} for that
layer list, in neighbours per node, and is $e$ wherever the cardinality is
uniform. \emph{Hard} is Eq.~\eqref{eq:hs} with the half rule and \emph{soft} is
Eq.~\eqref{eq:soft} by population dynamics, quoted to three decimals for the
reason Sec.~\ref{sec:hs-checks} gives. The last column is whether the two must
agree, which is cardinality two and nothing else. Generated by
\code{figs/\allowbreak hittingset.\allowbreak py:table\_\allowbreak five\_\allowbreak structures}.}
\label{tab:fivehs}
\end{table}

\paragraph{A graph} is one layer of links. One layer means the two sums of
Eq.~\eqref{eq:kcmulti} are proportional, so $(c-1)Y=1$ with $c=2$ gives $Y=1$
and the instability sits at $\ave{k}=e$ neighbours per node, which is
Eq.~\eqref{eq:kc}. For example, the row of Table~\ref{tab:fivehs} is a Poisson
link layer at chy-degree two, well below that. This is also where the two
problems coincide: at cardinality two,
hitting an edge and covering it are the same constraint, so this row is
Chapter~\ref{ch:cover}'s row: hard and soft agree, both returning the
Weigt--Hartmann size, and the instability is at $\ave{k}=e$.

\paragraph{A hypergraph} is one complex layer with the cardinality free. Where
it carries a single size $c$ the division is the same one and gives
$\ave{k}(c-1)=e$: the instability does not move at all, at
any cardinality, when it is counted in neighbours per node. For example, the
row is a Poisson layer of $3$-hyperedges at chy-degree one, the same two
neighbours per node as the graph. This is the structure this chapter was
written for, and
Sec.~\ref{sec:hardfail}'s counterexample lives inside it: disjoint triples,
where the hard field returns $1/2$ against a true $1/3$. The table's row is the
same failure at a density rather than on three vertices: $0.208$ against
$0.161$, an overstatement of about $29$ per cent, which is the same direction
and the same cause as Sec.~\ref{sec:soft}'s $55$ per cent for a $c=4$ layer at
$\ave{k}=1$. But the
instability does not move at all. One cardinality, however large, sits at $e$
neighbours per node, which is Eq.~\eqref{eq:kc} and is the first thing the
division after Eq.~\eqref{eq:kcmulti} gives. So this structure is on the wrong
side of one line and the right side of the other, which is the distinction the
section exists to make.

\paragraph{A multiplex network} is $L$ link layers on the same vertices. Every
$c_{l}$ is two, so the two sums of Eq.~\eqref{eq:kcmulti} are again
proportional and the instability is at $\sum_{l}\kappa_{l}=e$: the total
chy-degree, however many link types there are and however the degree is split
between them. For example, the row is two Poisson link layers at chy-degree one
each. A multiplex is a graph as far as this chapter can tell --- every layer is
a link layer, hitting a link is covering it, and what is being solved is vertex
cover of the union of the layers. Nothing in Sec.~\ref{sec:hardfail} applies
and the hard field is exact. That is Sec.~\ref{sec:fivecover}'s conclusion
reached by the other route and for the same reason, which is that the Jacobian
has rank one.

\paragraph{Interacting graphs and hypergraphs} is $L$ layers, and here the
cardinalities need not agree. Where they do, all equal to $c$, the division goes
through as before and $\bigl(\sum_{l}\kappa_{l}\bigr)(c-1)=e$; where they do
not, nothing cancels and Eq.~\eqref{eq:kcmulti} must be solved as a pair. For
example, the two rows are three link layers at chy-degree $2/3$ and three layers
of $3$-hyperedges at chy-degree $1/3$, both uniform and so both at $e$. This is
where this chapter's two partitions visibly come apart, and the only structure
that shows it. For
the hard field the rule is Chapter~\ref{ch:cover}'s: any layer above cardinality
two loses it, cross layer included. For the instability the cross layer matters
in a different way --- not whether it is above two, but whether it matches. Two
hypergraphs of cardinality three interacting through a cross layer also of
cardinality three sit at $e$ neighbours per node, the list being uniform. Give
the cross layer pairwise links instead and the list is mixed and the point moves
to $3.31$; make it cardinality four and it moves to $2.86$. The same edit to the
same layer crosses one line and not the other.

\paragraph{A clustered graph} is a link layer together with one layer per clique
size present. Two cardinalities or more leave the two sums of
Eq.~\eqref{eq:kcmulti} out of proportion, so there is no division to make and no
closed form: the pair is solved, and the answer depends on how the degree is
split between the layers.
For example, the row is a Poisson link layer at chy-degree one beside a Poisson
triangle layer at chy-degree one half. It is the only one of the five whose
layer list is mixed by construction, and so the only row whose instability is
not $e$. It sits at $3.28$ neighbours per node
instead. Two remarks. Its hard field is wrong, as every row with a layer above
cardinality two is, but mildly --- $0.310$ against $0.299$, four per cent
against the hypergraph row's twenty-nine --- because the link layer carries most
of the weight. That is the reassurance Sec.~\ref{sec:soft} ends on, met again. And
hitting a triangle is not what Part~III wants from a clustered graph. It asks
for one of the three vertices where Chapter~\ref{ch:cover} asks for two: the
same chygraph, two problems, and it is Chapter~\ref{ch:cover}'s that Part~III
uses.

\paragraph{What the list establishes.} What Sec.~\ref{sec:fivecover} establishes,
and one thing more. The same: that the differences between the five are
substitutions into one map, which is Chapter~\ref{ch:chygraphs}'s claim and is
what six rows of one table amount to. The more: that where the substitution
lands depends on which question is asked of it. Cardinality two is the line for
the hard field, in both chapters. Cardinality \emph{spread} is the line for this
chapter's instability, and it is not the same line --- a reader carrying
``above cardinality two things break'' across from Chapter~\ref{ch:cover} would
put the hypergraph row on the wrong side of it.
...
has stopped being right, and the second was a negative entropy.
Chapter~\ref{ch:ising} supplied the first, the stability of the symmetric
solution, as the de Almeida--Thouless line of Sec.~\ref{sec:atline}. This
section supplies the second, which differs from a stability line in saying not
only that the replica-symmetric answer is wrong but in which direction.
...
The one-step replica-symmetry-breaking calculation is not done here.
...
is not attempted. Equation~\eqref{eq:rsbansatz} is what would have to be solved,
and Sec.~\ref{sec:replicas-chy} is where it comes from. What this chapter can say is where the replica-symmetric
...
eleven digits for $c=2,\dots,20$. Equation~\eqref{eq:kcmulti} and the several-layer
reading of Eq.~\eqref{eq:kc} are new too, and are checked against
\code{rsb\_\allowbreak point} to $10^{-8}$ on each of Table~\ref{tab:fivehs}'s
six layer lists; the table's three cardinality-two rows are checked to agree
hard with soft, which is the one place they must. The failure is checked against arithmetic
