...
Several of the results are not what one would guess. A triangle transmits more magnetisation per
...
A word on what is old and what is not, since the two are mixed and the reader
is entitled to know which is which without reconstructing it.
...
Everything else appears here for the first time. That includes the critical
...
set problem, vertex cover, colouring and satisfiability; and the treatment of
overlapping complexes in Chapter~\ref{ch:overlap}. Where a chapter or a section
is new, it says so at its opening rather than leaving the reader to infer it,
and where a result of mine is corrected here, the correction is stated as one.
The published thresholds that Chapters~\ref{ch:colouring}
and~\ref{ch:satisfiability} benchmark against are other people's, attributed at
each use, and the last section of each of those chapters computes them rather
than quoting them. All of them can be reproduced
from two software packages. Every number the book computes comes out of them,
and the handful quoted from published work for comparison is attributed where it
appears; the chapter that follows this preface says which routine computes which
