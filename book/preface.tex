...
That is the trade the book explores. Give up the graph as the fundamental
...
Where the trade fails, the failure is structural rather than technical, so it
...
and they say so and cite as they go. So is one section outside them:
Sec.~\ref{sec:vw} recapitulates the transition \citet{vazquez2003vw} find in
the ensemble of graphs with arbitrary degree distribution and arbitrary degree
correlations, because Chapter~\ref{ch:cover} is measured against what that
description can do.
...
Part~IV, where overlapping complexes are measured and repaired ---
Sec.~\ref{sec:vw} excepted, and what is new in it is the measurement rather
than the transition. Where a chapter or a section
