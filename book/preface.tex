...
vertex covers; with a distribution over $q$ colours, graph colouring. And once a
complex is allowed to hold other complexes rather than only nodes, formulas that
no flat encoding can hold without damage. Through all of it the recursion does not change. Only what the
...
can start at Chapter~\ref{ch:chygraphs}. As in this author's earlier books, the longer
...
Part~II draws on published work \citep{vazquez2023pre,vazquez2024comnet}.
Part~III is newer, and two of its chapters --- colouring and satisfiability ---
have no paper behind them at all: their calculations were done for this book and
are checked here against the published thresholds. All of them can be reproduced
from two software packages, and every number quoted in the book comes out of
them; the chapter that follows this preface says which routine computes which
equation, and which test checks it.
