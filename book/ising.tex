...
Heat a magnet and at one definite temperature it stops being a magnet. Below
that temperature its spins agree with one another across distances enormously
larger than the range over which any of them interacts; above it they do not.
Lenz handed the model that describes this to Ising as a thesis problem. Ising
solved it in one dimension, found no transition anywhere, and concluded that the
model did not describe magnetism at all --- and it took Onsager, two decades
later, to show that in two dimensions the transition is there and to compute it
exactly. The disagreement was never about the model. It was about the structure
the model was put on.

That is why the Ising model is here, and it is not for its magnetism. It is the
smallest model in which a complex has something non-trivial to transmit, and the
smallest one in which the structure it is put on can be varied while everything
else is held fixed.
...
Here is the equation the rest of this section solves, written out in full,
because $u'$ is about to do all the work and it was last seen two chapters ago.
Put Eq.~\eqref{eq:Hising} inside Eq.~\eqref{eq:emitted}. A complex of
cardinality $c$ couples all $\binom{c}{2}$ pairs among its members, so the field
it emits onto member $0$, given the cavity fields $h_{1},\dots,h_{c-1}$ the
other members send in, and the derivative of that field at the trivial fixed
point where all of them vanish, are
\begin{equation}
  \begin{aligned}
    u'&=\frac{\partial u}{\partial h_{1}}\bigg|_{h=0},
      \qquad
      u=\tfrac12\ln\frac{\bar{Z}(\sigma_{0}=+1)}{\bar{Z}(\sigma_{0}=-1)},\\[4pt]
    \bar{Z}(\sigma_{0})&=\!\!\!\sum_{\sigma_{1}\dots\sigma_{c-1}=\pm1}\!\!\!
      e^{\,\beta J\sum_{0\le i<j\le c-1}\sigma_{i}\sigma_{j}
         \,+\,\sum_{j\ge1}h_{j}\sigma_{j}}.
  \end{aligned}
  \label{eq:uising}
\end{equation}
the derivative being the same one for each of the $c-1$ members by symmetry.
That is Eq.~\eqref{eq:uprime} with an Ising interior; the sum over
$2^{c-1}$ configurations and the derivative are the whole content of this
section. The number that comes out is everything a traversal of the complex
carries into Eq.~\eqref{eq:branch}, and the only place the Hamiltonian enters
the transition at all --- the rest of the threshold is counting.

Take $c=2$ explicitly, both because it has to come out as a link and because
every other cardinality is the same three steps with a longer sum. There is one
other member and one bond, so the sum has two terms:
\[
  \bar{Z}(\sigma_{0})=\sum_{\sigma_{1}=\pm1}
    e^{\beta J\sigma_{0}\sigma_{1}+h_{1}\sigma_{1}}
  =2\cosh(\beta J\sigma_{0}+h_{1}).
\]
That is the sum done. Put it in the logarithm, the factors of $2$ cancelling in
the ratio and $\cosh$ being even, so that $\sigma_{0}=-1$ turns
$-\beta J+h_{1}$ into $\beta J-h_{1}$:
\[
  u=\tfrac12\ln\frac{\cosh(\beta J+h_{1})}{\cosh(\beta J-h_{1})}.
\]
That is the field, for any incoming $h_{1}$. Now differentiate it,
\[
  \frac{\partial u}{\partial h_{1}}
    =\tfrac12\bigl[\tanh(\beta J+h_{1})+\tanh(\beta J-h_{1})\bigr],
\]
and evaluate at $h_{1}=0$, where the two terms coincide:
\[
  u'=\tanh\beta J.
\]
So the abbreviation $t=\tanh\beta J$ used from here on is not a change of
variable imposed on the answers afterwards. It \emph{is} the answer at $c=2$,
which is why the larger complexes are worth writing in it: each of them says
what a link would have transmitted, times a correction.

In that variable the interior sum gives
\begin{equation}
  \begin{aligned}
    c=2:&\quad u'=t,\\
...
\[
  u'=\frac{\partial}{\partial h_{1}}\tfrac12\bigl[\ln \bar{Z}(+)-\ln \bar{Z}(-)\bigr]
     \bigg|_{h=0}
...
\hline\hline
\end{tabular}
\caption{\textbf{One clique layer at chy-degree two.} The critical coupling from
$\ave{\kbar}(c-1)u'(c)=1$, and the de Almeida--Thouless coupling from the same
condition with $u'$ replaced by $u'^{2}$, against cardinality. The first column
is the check: at $c=2$ the complex is a link, the condition is
$\tanh\beta J_{c}=1/\ave{\kbar}=1/2$, and $0.5493$ is the Bethe-lattice value.
Raising the cardinality lowers the coupling needed to order, by a factor of
$2.6$ between a link and a triangle, because a clique both transmits more per
traversal and offers $c-1$ members instead of one --- here the chy-degree is
held fixed, so the compensating loss of branches that
Sec.~\ref{sec:clustering} turns on is absent and only the gain is left. The two
rows also separate as $c$ grows, from a ratio of $1.60$ at $c=2$ to $2.21$ at
$c=6$: the larger the complex, the wider the window in which a ferromagnet is
ordered and a spin glass on the same structure is still stable. Generated by
\code{figs/\allowbreak ising.\allowbreak py}.}
\label{tab:cliquetc}
\end{table}
...
exactly as the temperature gap does. Figure~\ref{fig:magnetisation} draws both
quantities for the matched pair at degree four, against the Wolff simulations
of Eq.~\eqref{eq:mc}.
...
\[
  \bar{Z}(\sigma_{0})=\prod_{j=1}^{q-1}\bigl(2\cosh h_{j}\bigr)
