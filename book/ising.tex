...
Chapter~\ref{ch:statmech} writes the same field $\mu$, which is its own
Eq.~\eqref{eq:up}. Where a $B$ carries two layer indices it is the branching
matrix of Eq.~\eqref{eq:branch} and not a field.
...
Eqs.~\eqref{eq:magamp} and~\eqref{eq:chiamp}. Substituting the Ising interior
into them is this section, and it costs what substituting into
Eq.~\eqref{eq:branch} cost in Sec.~\ref{sec:tc}: an enumeration and nothing
else. The external field written $\mu$ there is the $B$ of
Eq.~\eqref{eq:Hising} here.
