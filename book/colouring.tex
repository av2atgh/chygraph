...
produced one of them. This section produces the other two.
...
condensation transition is outside it; the treelike condition of
Eq.~\eqref{eq:up} is untouched; and what the chygraph adds is that
Eqs.~\eqref{eq:colsp} and~\eqref{eq:colsigma} are written for an arbitrary
chy-degree distribution, so that the cardinality result of
Sec.~\ref{sec:twoconstraints-col} carries over as a substitution. That the
graph case reproduces published constants is a check on the implementation, and
it is the reason to trust the substitution rather than a result in itself.
