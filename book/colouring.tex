...
entirely. This section asks what that means for a clustered graph, which is the
...
\paragraph{How large is the effect.} One in degree --- and only for regular
ensembles. For Poisson layers $\ave{\kbar}=\ave{\kappa}$, so the cross term
...
$f$. There is no difference at all.
...
is the whole of the effect. It is the same lost branch that separates
...
Poisson triangle layer, both routes return $(q-1)^{2}$, and the graph
calculation gives the right answer. It does so by accident. Its independence
assumption is exactly as false as before; what saves it is that the quantity it
miscounts --- the excess-degree correction --- is one a Poisson degree
...
\paragraph{One exclusion, and it is real.} At $q=2$ with $c=3$ and $c=4$
...
\section{Extending the calculation to chygraphs}
\label{sec:colouring-cq}
\index{survey propagation}

Reference~\citep{krzakala2004} solves the one-step calculation for colouring on
a graph and locates $c_{q}$, the density at which proper colourings run out.
This section writes that calculation for a chygraph and runs it on a network of
triangles.

\paragraph{The extension.} Their survey update, their Eq.~(16), combines
messages arriving along edges; their complexity, their Eq.~(20), is a site term
...
expressions with the chy-degree distribution in place of the degree and a
contribution per complex in place of the link term, the complex's interior
enumerated at whatever cardinality it has. The shape of both is unchanged: this
is Chapter~\ref{ch:statmech}'s substitution applied to the one-step calculation
rather than to the cavity recursion.

At $c=2$ the extension must return the original, and it does. Writing
$\eta=e/q$, the update becomes their Eq.~(16) term for term and
Eq.~\eqref{eq:colsigma} becomes their site sum together with
$-\tfrac{\ave{k}}{2}\ln(1-q\eta_{1}\eta_{2})$, which is why
Sec.~\ref{sec:colouring-sp} returns Mulet's $c_{q}$ to a tenth of a per cent.
...
crossing, and the colourable phase is the trivial region alone --- the threshold
is the lift-off. Note the vertical scales, which differ by a factor of six: the
graph's arc peaks at $+0.07$ where the triangle branch falls to $-0.4$. Averaged
...
than one cluster, so the colourings run out at the same density where they would
have shattered. One boundary instead of two, and it arrives earlier ---
...
\hline
\multicolumn{4}{@{}l}{\rule{0pt}{2.6ex}\itshape colourability threshold $c_{q}$, in degree}\\
$3$ & $4.69$ & $3.083$ & $-34.3\%$\\
$4$ & $8.90$ & $6.771$ & $-23.9\%$\\
\multicolumn{4}{@{}l}{\rule{0pt}{2.6ex}\itshape replica-symmetric stability line, Eq.~\eqref{eq:tridisagree}}\\
$3$ & $5$ & $6$ & $+20\%$\\
$4$ & $10$ & $11$ & $+10\%$\\
\hline\hline
\end{tabular}
\caption{\textbf{The two lines move in opposite directions.} Above, where
proper colourings actually run out, from Eq.~\eqref{eq:colsigma}; the graph
column is Mulet's and the triangle column is computed here. Below, where the
replica-symmetric solution loses local stability. Promoting links into triangles
at fixed degree raises the second and lowers the first. Generated by
\code{figs/colouring.py:check\_clustered\_cq}.}
\label{tab:cq}
\end{table}

\paragraph{Triangles make colouring harder.} Table~\ref{tab:cq} is the result.
A triangle-clustered graph stops being $3$-colourable at degree $3.08$ where a
triangle-free one survives to $4.69$, and stops being $4$-colourable at $6.77$
against $8.90$: a third and a quarter of the range, gone.

Set that beside Sec.~\ref{sec:colouring-triangles}, where the replica-symmetric
stability line was found to move the other way, from $(q-1)^{2}+1$ to
$(q-1)^{2}+2$. Both are correct and they are different lines --- one is where a
solution stops being locally stable, the other where solutions stop existing ---
but the consequence is worth stating flatly. On a graph the two sit within six
per cent, so one serves as a proxy for the other. On triangles $c_{q}$ is half
the stability line, and a reader who takes the proxy will conclude that
clustering makes colouring \emph{easier}. It makes it harder, by a third.
...
fraction is. It is Ref.~\citep{krzakala2004}'s $e(y)$, which needs the
