...
closed forms that are worth the chapter on their own.
...
change, and it is worth flagging before the algebra, is the \emph{message}. Up
to now a message has been one number --- a probability, a field, an integer, or
...
\paragraph{Why the cardinality cancels, in words.} It is worth saying without
the algebra, because the result is the chapter's least expected. What one member
...
another; for hypergraph colouring (dashed) the transmission collapses like
$q^{-c}$. Right: the resulting local-stability threshold in neighbours per
...
neighbours it supplies move in exactly compensating ways --- and it is worth
noticing that this is the \emph{opposite} of what happened for the Ising model,
where Sec.~\ref{sec:clustering} found the two effects failing to compensate and
the multiplicity winning.

\section{The two constraints diverge above cardinality two}
\label{sec:twoconstraints-col}

Equation~\eqref{eq:coltwo} is the sharpest statement in the chapter, so it is
worth being explicit about what it says.
...
Read that table carefully, because it says something uncomfortable. At $q=3$ the
local instability sits inside the colourable phase, below both the clustering
...
one-step calculation --- with survey propagation and the iterative algorithms
built on it \citep{braunstein2003}, and, for the rigorous side of the same
question, Ref.~\citep{cojaoghlan2016} --- --- survey propagation, a complexity function, and the
condensation and rigidity transitions that Ref.~\citep{zdeborova2007} maps out.
...
Until it is done, the honest reading of Eq.~\eqref{eq:coltwo} is as a statement
