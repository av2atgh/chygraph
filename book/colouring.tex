...
\section{The proper rule closes the window}
\label{sec:colouring-window}
\index{survey propagation}

The proper rule is absent from Table~\ref{tab:hypsp} because the substitution of
Eq.~\eqref{eq:hypsp} does not reach it. A set-valued survey does, and the point
of this section is what happens when one is carried: the machinery works, and
the answer is that at cardinality three there is nothing for it to find.

\paragraph{Carrying the set.} The message from a complex to a member is the set
of colours still open to it, and a survey is a distribution over such sets taken
over clusters. Colour symmetry keeps this cheap --- a survey is uniform within
each size class, hence a $(q+1)$-vector --- and the interior is the condition
under which the other $c-1$ members can be given distinct colours avoiding a
given one, which is Hall's condition and is enumerated once. Two checks say the
apparatus is sound. At $c=2$ it returns Mulet's $c_{q}$: $4.68$, $8.86$, $13.64$
against $4.69$, $8.90$, $13.69$. At $c=3$, with the hypergraph rule substituted
for the proper one and nothing else changed, it returns
Ref.~\citep{gabrie2017}'s thresholds, the complexity crossing zero at
$\ave{\kappa}=26.92$ and $63.3$ for $q=3$ and $4$.

\paragraph{What it finds for triangles: nothing to find.} A threshold needs an
interval in which the clusters both exist and number more than one --- where a
non-trivial survey has appeared and $\Sigma$ is still positive. On a graph that
interval is where $c_{q}$ lives. On a network of triangles it does not exist:
\begin{table}[t]
\centering\small
\begin{tabular}{lcccc}
\hline\hline
 & \multicolumn{2}{c}{$q=3$} & \multicolumn{2}{c}{$q=4$}\\
 & graph & triangles & graph & triangles\\
\hline
branch appears at & $4.41$ & $3.08$ & $8.08$ & $6.77$\\
best $\Sigma$ above it & $+0.026$ & $-0.137$ & $+0.049$ & $-0.241$\\
window & to $4.69$ & none & to $8.90$ & none\\
\hline\hline
\end{tabular}
\caption{\textbf{Where the one-step calculation has somewhere to stand.} The
degree at which a non-trivial survey first appears, the largest complexity
found above it, and the interval over which the complexity stays positive. Degrees, not
chy-degrees, so the two constructions are compared at the same number of
neighbours. The graph windows close at the published colourability thresholds,
$4.69$ and $8.90$; the triangle columns have no window to close. Generated by
\code{figs/colouring.py:check\_window\_closes}.}
\label{tab:colwindow}
\end{table}

Table~\ref{tab:colwindow} sets the four cases side by side. In both graph columns the branch arrives with $\Sigma$
positive and the complexity then falls through zero at the published
colourability threshold. In both triangle columns it arrives with $\Sigma$
already negative --- fewer than one cluster --- and never returns.

\paragraph{What that does and does not mean.} It does not mean a
triangle-clustered graph has no colourability threshold. It means $m=0$ cannot
locate it. Survey propagation counts clusters without regard to their size,
which is the right weighting when the transition is where the last cluster
disappears and the wrong one when the clusters have already stopped being
equally worth counting by the time any exist.

The repair is to reweight the clusters instead of counting them equally, and it
is worth saying what that costs rather than leaving it as a name. The apparatus
is standard and is set out in full for this very problem by
Ref.~\citep{krzakala2004}, whose stability criterion Sec.~\ref{sec:colouringthreshold}
already borrows: clusters carry a factor $e^{-yE}$, the complexity becomes a
function $\Sigma(y)$ of that reweighting, and the limit $y\to\infty$ --- which
concentrates on zero-energy ground states --- is the calculation done above.
Everything in Table~\ref{tab:colwindow} lives at that one endpoint. Moving off
it means every message carries a free-energy shift alongside its warning, the
population is reweighted by it, and the reweighting is scanned rather than
fixed. That is a heavier object than anything else in this book.

What it is not is unverifiable. Reference~\citep{gabrie2017}'s Table~1 gives
condensation and rigidity thresholds at $m=1$, and Ref.~\citep{zdeborova2007}
gives the condensation line for graph colouring; either would gate a finite-$y$
calculation at cardinality two exactly as Mulet's $c_{q}$ gated the one above,
before it was turned on triangles. The path is open and the benchmarks exist.

The reason it is not taken here is scope. This book's claim is about the
recursion --- one map, carrying whatever the model requires, over complexes that
may contain complexes. The reweighting changes how the population is
weighted, not what travels along an inclusion, so a finite-$y$ treatment would
be Ref.~\citep{krzakala2004}'s machinery bolted onto the object rather than a
statement about it. That is a good reason to leave it out and a poor one to pretend the question
is closed: what Table~\ref{tab:colwindow} reports is the failure of a particular
weighting, not the absence of a threshold.

The contrast in the table is what makes this a statement about the constraint
rather than about cardinality. Same machinery, same $q$, same cardinality three:
with the hypergraph rule the window is there and the threshold comes out, and
with the proper rule it is gone. Section~\ref{sec:twoconstraints-col} found the
two rules identical at cardinality two and three orders apart above it in what a
complex transmits. Here they are identical at cardinality two and differ, above
it, in whether the one-step calculation has anywhere to stand.

This is the second time the book has met that shape. Section~\ref{sec:sat-sp}'s
qualifications end with the same one, and the nested clauses of
Sec.~\ref{sec:nested} behave the same way when the same calculation is attempted
on them: a branch arriving at finite $\Sigma$, and no crossing. Two problems
that share nothing but a cardinality above two.
