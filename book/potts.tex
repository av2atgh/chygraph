...
\[
  v^{4}\,(q-1)^{3}\,(q-v)^{2}\,\bigl(v^{2}-4q+4\bigr)=0.
\]
The factor $(q-v)^{2}$ is the disordered spinodal $v_{c}=q$ already found from
Eq.~\eqref{eq:branchcond}; the last factor is the fold, so the ordered branch
first exists at
\begin{equation}
  v^{*}=2\sqrt{q-1}.
  \label{eq:fold}
