...
\end{table}
Both spinodals are exact. The disordered one is Eq.~\eqref{eq:branchcond}
itself, $v_{c}=q$ at excess degree $2$; the fold is where the same
fixed-point equation acquires a double root, and eliminating the message
between it and its derivative leaves $v^{2}-4q+4=0$, so the ordered branch
first exists at
\begin{equation}
  v^{*}=2\sqrt{q-1}.
  \label{eq:fold}
