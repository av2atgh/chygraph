\chapter{Random-cluster model on chygraphs}
\label{ch:potts}
\index{Potts model|textbf}
\index{random-cluster model}

Fortuin and Kasteleyn introduced the random-cluster model to establish the
relations between a family of models that had been studied separately
\citep{fortuin1972}. The Ising model, percolation, graph colouring and linear
electrical networks belonged to different communities, and the resemblances
between them had been noticed one pair at a time: percolation's likeness to the
Ising model was remarked on in the 1950s and made precise only in 1968; the
number of $n$-colourings of a graph is the ground-state degeneracy of the
antiferromagnetic Potts model; and Tutte had shown that Kirchhoff's
spanning-tree generating function and Birkhoff's chromatic polynomial are two
faces of one two-variable polynomial. The random-cluster model was, in their
words, the final step in establishing those relations, and it embodies the
entire family: one measure on the edges of a graph, carrying one real parameter,
with the members recovered by fixing it. At $q\to0$ it is the electrical
network, at $q=1$ percolation, at $q=2$ Ising, and at $q=n\ge2$ the Potts model
with $n$ states and the $n$-colouring problem. Fortuin and Kasteleyn write that
parameter $\kappa$; this book keeps $\kappa$ for the chy-degree and writes the
random-cluster parameter $q$ throughout.

This chapter has the same intent one level up. Part~II solved percolation on
chygraphs and Part~III solves the Ising model, the hitting set problem, vertex
cover, colouring and satisfiability on them, and the two have been presented as
separate subjects sharing a method. They are one subject: the percolation map of
Chapter~\ref{ch:percolation} is the $q\to1$ limit of the cavity recursion for
the $q$-state Potts model, complex by complex, and the Ising model of
Chapter~\ref{ch:ising} is the same recursion at $q=2$ --- the same equations, at
two values of one parameter. The identity is Fortuin and Kasteleyn's and is not
new; what is new is that it holds complex by complex on a chygraph, and the
transmission factor that makes it do so. Section~\ref{sec:qlimit} does the
limit and Sec.~\ref{sec:transmission} exhibits the single function that both
chapters have been computing without saying so.

\section{The random-cluster model}
\label{sec:rc}
\index{random-cluster model|textbf}
...
The first statement of this chapter needs no machinery:
...
\label{sec:fk}
\index{Fortuin--Kasteleyn correspondence|textbf}
...
Eq.~\eqref{eq:fk} and the dictionary $p=1-e^{-\beta J}$. Nothing computes Eq.~\eqref{eq:fk} --- it is an identity --- but the two limits it licenses are checked in \code{figs/\allowbreak potts.\allowbreak py}, at $q\to1$ against the reachability generating function and at $q=2$ against the Ising cavity field.

Equation~\eqref{eq:fk} is the Fortuin--Kasteleyn representation
\citep{fortuin1972}. What it does and does not say is routinely garbled, so
state it exactly.
...
content. Fortuin and Kasteleyn's own numbering, listed at the head of this
chapter, is the right way to keep this straight. \emph{Bond percolation is the
$q\to1$ limit of the random-cluster model, not of the Potts model.}
...
the derivative: since $\partial q^{k(\omega)}/\partial q=k(\omega)q^{k(\omega)-1}$
and $Z_{\mathrm{RC}}=1$ at $q=1$,
\begin{equation}
...
Chapter~\ref{ch:percolation} wrote down
...
complex emits onto member $i$ the pair of sums $\bar{Z}_{a\to i}(\sigma_{i})$ with
$\sigma_{i}$ distinguished or not, and only their ratio
\[
  \rho_{a\to i}=\frac{\bar{Z}_{a\to i}(\sigma_{i}\ \text{not distinguished})}
                     {\bar{Z}_{a\to i}(\sigma_{i}\ \text{distinguished})}
\]
...
\[
  \bar{Z}(\text{not dist.})=(1+v)y_{j}+1+(q-2)y_{j}.
\]
...
from a connectivity argument. \code{figs/\allowbreak potts.\allowbreak py:percolation\_\allowbreak limit} does the general case symbolically, with an independent $y_{j}$ on every member so that no symmetry is being relied on.
...
already. That is why Part~II could come first.

\section{The transmission factor}
\label{sec:transmission}
\index{transmission factor|textbf}
...
\begin{equation}
  ...
  \label{eq:tau2}
\end{equation}
and this one expression is where the two parts of the book meet. At $q=1$ it is
...
The interior first matters at $c=3$, and the same two sums give it. One
simplification first: at the symmetric point $\bar{Z}_{a\to i}$ does not depend on
which state $\sigma_{i}$ is in, so the two partition functions are equal there
and the quotient rule collapses to
\begin{equation}
  \tau_{c}=\frac{Z'(\text{not dist.})-Z'(\text{dist.})}{Z}\bigg|_{y=1},
  \label{eq:taudiff}
\end{equation}
a difference of two derivatives over their common value.

The denominator is the interior sum of an isolated complex, and for a triangle
it is a count of four coincidence patterns, each weighted by $(1+v)$ raised to
the number of agreeing pairs it creates. Holding $\sigma_{i}$ and summing the
other two: all three agree, one way, three agreeing pairs; the other two agree
with each other but not with $i$, $q-1$ ways, one pair; one of them agrees with
$i$ and the other does not, $2(q-1)$ ways, one pair each; and all three differ,
$(q-1)(q-2)$ ways, no pair. So
\[
  Z=(1+v)^{3}+3(q-1)(1+v)+(q-1)(q-2)=q^{2}+3qv+3v^{2}+v^{3}.
\]
Carrying $y_{j}$ through the same four patterns and taking the difference of the
derivatives in Eq.~\eqref{eq:taudiff} leaves $v(q+3v+v^{2})$, so
\begin{equation}
...
\code{figs/\allowbreak potts.\allowbreak py}.}
...
\paragraph{Where the analogy stops.} Above $q=2$ the
...
state rather than the transition. The ordered branch already exists below it,
and the transition is neither edge of that coexistence window but the point
inside it where the two branches exchange which has the lower free energy. At
excess degree $2$, iterating the graph recursion from an ordered start finds the
lower edge and comparing the two branches' Bethe free energies finds the
crossing (Table~\ref{tab:pottswindow}).
\begin{table}[t]
\centering\small
\begin{tabular}{lcccc}
\hline\hline
...
transition & $2.000$ & $2.847$ & $3.522$ & $4.634$\\
ordered branch from & $2.000$ & $2.828$ & $3.464$ & $4.472$\\
$v_{c}$ too high by & --- & $5.1\%$ & $11.9\%$ & $22.8\%$\\
\hline\hline
\end{tabular}
\caption{\textbf{Above $q=2$ the branching condition is a spinodal.} The three points of the coexistence window on a graph of excess degree $2$: the disordered spinodal Eq.~\eqref{eq:branchcond}, the transition from a Bethe free-energy crossing, and the fold Eq.~\eqref{eq:fold} at which the ordered branch first exists. All three coincide at $q=2$. The last row normalises the error by $v_{c}$. Generated by \code{figs/\allowbreak potts.\allowbreak py:check\_\allowbreak transition\_\allowbreak point}.}
\label{tab:pottswindow}
\end{table}
Both spinodals are exact, and one elimination gives both. At excess degree $2$
the recursion is $y=\rho(y)^{2}$, with $\rho$ the emitted ratio of a link ---
the two sums above, divided:
\[
  \rho(y)=\frac{1+(q+v-1)\,y}{(1+v)+(q-1)\,y}.
\]
The ordered branch first exists where that fixed point is a double root, so
$y=\rho(y)^{2}$ and $1=2\rho(y)\rho'(y)$ hold together. Eliminating $y$ between
the two --- a resultant, since both are polynomial once cleared --- leaves
\[
  v^{4}\,(q-1)^{3}\,(q+v)^{4}\,(q-v)^{2}\,\bigl(v^{2}-4q+4\bigr)=0,
\]
up to an overall sign. Three of the five factors are artefacts of clearing the
denominators and cannot vanish for $q>1$, $v>0$: $v^{4}$ and $(q-1)^{3}$ are the
decoupled and percolation limits, and $(q+v)^{4}$ is the symmetric root $y=1$
meeting the second equation at $v=-q$, outside the physical range. The two that
remain are the spinodals. $(q-v)^{2}$ is the disordered one, $v_{c}=q$, already
found from Eq.~\eqref{eq:branchcond} --- it is $y=1$ itself going marginal ---
and the last factor is the fold, so the
ordered branch first exists at
\begin{equation}
  v^{*}=2\sqrt{q-1}.
  \label{eq:fold}
\end{equation}
Only the transition between them has to be located numerically.
At $q=2$ all three coincide and the transition is continuous, which is why
Chapter~\ref{ch:ising} may use the linear condition and why
Chapter~\ref{ch:percolation} may at $q=1$. Above it they do not: the linear
condition overstates the transition by $5.1$ per cent of $v_{c}$ at $q=3$ and by
$22.8$ per cent of it at $q=6$ --- the errors are quoted throughout as a
fraction of $v_{c}$, not of the transition, which would make them $5.4$ and
$29.5$ per cent, and the window it sits at the top of is wider still. This
...
The formalism does both things and they look alike, so separate them.
...
\section{Percolation and beyond}
\label{sec:percolation-beyond}

Percolation came first for simplicity. At $q=1$ the interior sum, which for a
general model is a sum over $q^{c}$
...
What Part~III adds is a heavier message rather than a new recursion. The
...
The linear threshold $v_{c}=q/(\ave{\bar k}-1)$ is solved symbolically and
confirmed to return $p_{c}=1/\ave{\bar k}$ at $q=1$, which is Molloy and Reed,
and $\tanh\beta J=1/\ave{\bar k}$ at $q=2$, which is the Bethe-lattice Ising
temperature. In Table~\ref{tab:pottswindow} the $q=2$ column agrees with
Eq.~\eqref{eq:branchcond} to four digits, which is what says the iteration is
right where the answer is independently known.

\checkhead{New results}
That the correspondence holds complex by complex, and the transmission factor
that carries it, are new here. Equation~\eqref{eq:qonelimit} is verified
symbolically in the bond probability, with an independent $y_{j}$ on each member
so that no symmetry is being relied on, for complexes that are $K_{2}$, $K_{3}$,
$K_{4}$, $K_{5}$ and paths on three and four vertices, against the reachability
generating function obtained by enumerating all $2^{|E|}$ bond configurations of
the interior. In the symmetric case it is checked against the independent
implementation of $\bar G$ used in
Chapters~\ref{ch:percolation}--\ref{ch:epidemics}. The other end of the axis is
checked the same way: at $q=2$, with $y_{j}=e^{-2h_{j}}$, the emitted ratio is
$e^{-2u}$ with $u$ the Ising cavity field, verified against an independent
enumeration of the Ising interior for $K_{2}$, $K_{3}$ and $K_{4}$. So the
interior sum reproduces Chapter~\ref{ch:percolation}'s at one end and
Chapter~\ref{ch:ising}'s at the other, and neither of those two was written with
the other in mind. Equations~\eqref{eq:tau2} and~\eqref{eq:tau3} are obtained by
differentiating that interior sum, and their limits
Eq.~\eqref{eq:tau3limits} are checked against the closed forms the two chapters
quote.

\checkhead{Partial results}
Above $q=2$ only the two spinodals are closed forms. The transition between them
is located numerically, by comparing Bethe free energies across the coexistence
window, and Table~\ref{tab:pottswindow} is that calculation at one excess degree
and four values of $q$ --- not a general result. The errors in its last row are
fractions of $v_{c}$, not of the transition, and the two conventions differ by
enough to matter.
