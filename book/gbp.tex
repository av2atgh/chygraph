...
When a method fails there are two things to do, and the second should be tried
first: find out whether somebody has already repaired it. Somebody has. Kikuchi
repaired this one in 1951, for reasons that had nothing to do with networks. He
wanted a better free energy for an alloy than the Bethe approximation gave, and
he got it by keeping the bookkeeping on clusters of sites rather than on single
bonds. Half a century later \citet{yedidia2005} showed that the Bethe free
energy is precisely what belief propagation minimises, that Kikuchi's is what a
more elaborate message passing minimises, and wrote down the algorithm that does
it.

That algorithm is the obvious candidate here, because the obstruction of
Chapter~\ref{ch:overlap} is exactly a cluster of sites that the pairwise
bookkeeping counts twice. Whether it is the right candidate is the question this
chapter answers, and the answer is not the expected one.

Chapter~\ref{ch:overlap} measured what $\mathrm{BP}(\mathrm{Ising},\chi)$
loses to
...
Which regions, and which counting numbers, is the whole of the construction, and
it is three lines.

\begin{model}{The region graph}
\begin{rules}
\rl{1}{Seed} Take the complexes of the chygraph as the initial regions.
\rl{2}{Close} Add the intersection of every pair of regions already present, and
repeat until no new set appears. The family is then closed under intersection.
\rl{3}{Count} Give each region $R$ the counting number fixed by M\"obius
inversion,
\begin{equation}
  c_{R}=1-\sum_{R'\supsetneq R}c_{R'},
  \label{eq:mobius}
\end{equation}
working downwards from the regions contained in nothing, which take $c_{R}=1$.
\end{rules}
\end{model}

What the rules produce is a family in which every variable and every interaction
is counted exactly once, which is the property a tree had for free and the
property Eq.~\eqref{eq:mobius} is engineered to restore. Rule~2 is the expensive
one: nothing bounds the size of the closure in advance, and
Sec.~\ref{sec:realoverlap} is where that bites.

The counting numbers are what a free energy is built from. Evaluated on regions
treated as isolated --- an estimate, not yet a fixed point --- it is
$-\beta F=\sum_{R}c_{R}\ln Z_{R}$, with $Z_{R}$ the exact partition function of
region $R$ on its own; that sum is
\code{gbp.\allowbreak static\_\allowbreak log\_\allowbreak Z}.
Equation~\eqref{eq:bethe} carries the same counting on one particular family,
$c=1$ on every complex and $1-k$ on every node. What it does not carry is the
same $Z_{R}$: its terms hold cavity fields rather than isolated regions, and it
is Eq.~\eqref{eq:para}, the same free energy at zero field, that is this
expression on that family. The two schemes compared below differ in their
$c_{R}$ and in nothing else.
...
\emph{It departs from it exactly where complexes overlap.} Run
Eq.~\eqref{eq:mobius} on two triangles sharing an edge. The family closes on
$\{0,1,2\}$, $\{1,2,3\}$ and their intersection $\{1,2\}$; neither triangle is
contained in anything, so each takes $c_{R}=1$, and the shared edge, contained in
both, takes $c_{\{1,2\}}=1-(1+1)=-1$. The Bethe counting instead puts $-1$ on
each of the two shared \emph{nodes}, each lying in two complexes.

Summing the counting numbers of every region containing a given object is the
test, and on nodes both schemes pass it: node $1$ gives $1+1-1=1$ either way.
They part on the shared \emph{bond} $\{1,2\}$, which lies inside both triangles.
Under Eq.~\eqref{eq:mobius} the region that subtracts it exists and the total is
$1+1-1=1$; under Eq.~\eqref{eq:bethe} no region below a complex holds a pair,
and the total is $2$. The Bethe scheme subtracts the two shared vertices
independently and so never removes the correlation between them.
\code{region.\allowbreak RegionGraph} builds the family and its counting, and
its \code{counting\_\allowbreak is\_\allowbreak valid} is the test just run.
...
contains Eq.~\eqref{eq:bethe}, and Sec.~\ref{sec:gbp-checks} reports the
numerical form of that statement. \code{Chygraph.\allowbreak region\_\allowbreak gbp} runs Eq.~\eqref{eq:p2c} on the
family \code{Chygraph.\allowbreak regions} closes; the two are \code{gbp.\allowbreak GBP}
and \code{region.\allowbreak RegionGraph}.
...
its own assumption. \code{Chygraph.\allowbreak regions} closes the family and
reports whether it is a junction tree, which is the test the rest of the chapter
turns on.
...
$\mathrm{BP}$ but not near zero, and the two do not track: over the
...
\caption{\textbf{GBP against BP.} Error in $\ln Z$ for
$\mathrm{BP}(\mathrm{Ising},\chi)$ --- the cavity recursion on the clique
chygraph --- and for generalised belief propagation on the same $240$
runs, against the axis of Fig.~\ref{fig:cavityfail}. GBP is exact on the chordal
instances and improves on $\mathrm{BP}$ nearly everywhere else --- median
