...
Chapter~\ref{ch:overlap} measured what the cavity method loses to non-trivial
overlap. This chapter runs the repair the literature already has ---
Kikuchi's, in the message-passing form given by \citet{yedidia2005} and
introduced in Sec.~\ref{sec:gbp-intro} --- on the same three classes of network,
and measures what it recovers and where it stops.
...
Generated by \code{figs/merge.py:check\_placed\_finite\_size}.}
\label{tab:knfinite}
\end{table}
...
ensemble and $56$ of $180$ on the real neighbourhoods. The placed ensemble is
...
None of this is a measurement of the Karrer--Newman ensemble. It is a
measurement of what the messages do on small dense graphs that happen to have
been generated by placing triangles, and Table~\ref{tab:knfinite} is the reason
the distinction matters. What the ensemble does at its own scale is
Sec.~\ref{sec:merging}'s question, and there it behaves as advertised.
...
and in neighbourhood density alike. Data from \code{probe/gbp\_real.py}; plotted
by \code{figs/overlap.py}.}
\label{fig:gbpreal}
\end{figure}
...
\begin{calculation}{why provenance and not amount}
...
Chapter~\ref{ch:data}'s statement about $\bar s$ from the other direction.
\end{calculation}

\paragraph{A small residual stops certifying an answer.} Of the $64$ non-chordal runs, $37$ drive the residual below $10^{-9}$.
On the hyperbolic instances every converged run also satisfied
$\sum_{x_{P}\setminus x_{R}}b_{P}=b_{R}$ to $2\times10^{-6}$. Here $7$ of the
$37$ stop moving without the beliefs agreeing, by margins as large as
$2\times10^{-2}$: the messages reach a state they do not leave, and it is not a
...
both tests reproduce to $2\times10^{-12}$ in $\ln Z$. The $7$ that reach a small
...
$12$ of them, error from $7\times10^{-14}$ to $1\times10^{-4}$, four
...
$-\ln 2$ to eight decimal places---the signature of a fixed point that has
...
$5\times10^{-13}$ and a consistency of $3\times10^{-15}$---as clean a fixed
...
non-chordal runs only $30$ yield a fixed point that passes both tests, and $7$
of the $37$ that stop moving do so without the beliefs ever agreeing. Four of
...
that ensemble is not in the régime that defines it (Table~\ref{tab:knfinite}),
