...
On the other $142$ it does not recover the failure. On the hyperbolic instances
it converges on $14$ of $20$ and gains two to nine orders where it does. On the
placed instances the gain is $-0.1$ to $1.7$ orders, median $0.3$. On the real
neighbourhoods $30$ of $64$ reach a fixed point that is both converged and
consistent, and four of those $30$ are no better than the static counting they
were meant to improve, the worst by a factor of nine.

The scheme is therefore exact where the chygraph was already close to exact, a
chordal clique family being one whose incidence structure is a tree, and where
the chygraph fails it fails too. Chapter~\ref{ch:metacomplex} takes the other
route.
