...
gaps are specific enough to be worth someone's time.

\section{One recursion, many models}
\label{sec:onerecursion}

The through-line is worth seeing in one place (Table~\ref{tab:through}). Read
down the columns rather than across the rows: the recursion never changes. What
...
the rest of the book barely uses. Section~\ref{sec:flatteningcost} measures what
it is worth: a nested clause has a treelike three-level representation, its CNF
flattening does not, and a flat treatment misplaces the transition by seven to
twenty-two per cent. That the deficit is manufactured by the \emph{encoding}
rather than found in the data is the sharpest form of
Chapter~\ref{ch:giant}'s moral --- the tree calculation applied to the wrong
object.
...
Neither of these was planned. Both turned up in three or four independent
places, which is the only reason to trust them.
...
triangle construction carries $\ave{\kbar}=n+1$ rather than $n$.
...
branching condition sits $5.7$ to $25.5$ per cent above the fold at $q=3$ to
$6$. And Sec.~\ref{sec:unanimity} met it where it does real damage, in the
unanimity interaction, and there the free energy built in
Sec.~\ref{sec:freeenergy} finally had to be used.

The practical corollary is the one worth carrying away, because it is easy to
get wrong in the direction that flatters the calculation: critical slowing down
mimics a coexistence window. An ordered branch that survives a finite iteration
...
gap and Chapter~\ref{ch:overlap} is entirely about it. The instance calculation
is done and exact where the clique structure is chordal; what is missing is a
...
share more than one node. So the honest summary is that the chygraph moves the
boundary of what counts as local, from ``no loops'' to ``no loops between
