...
criterion saying when it will --- which on a hyperbolic random graph is only in
the very sparse r\'egime, though Sec.~\ref{sec:placed} finds it terminates at
every density on an ensemble whose motifs are placed rather than grown, at the
same clustering coefficient. Whether the repair is affordable turns out to be a
question about where a network's overlaps come from and not about how clustered
it is. What is missing is a
