...
Let me start with a system everyone reading this book belongs to: the scientific
literature. It is a good place to start because it has been mapped many times
over, because the maps disagree with one another, and because the reason they
disagree is exactly the subject of this chapter.

Here is what is actually there. A paper is written by some authors. It cites
...
Worse, they throw away different things, so a result proved on one of them
cannot be quoted on another. This is not a defect of any of the four. It is what
happens when a system whose parts contain other parts is forced into a language
...
Now look at what a single paper is. It is not a node, and it is not a link
either. It is a small structured thing in its own right: it has an author list,
...
That last sentence is doing more work than it looks. Suppose we agree to call
the paper one object and its two lists the internal structure of that object.
...
that paper cites, you have to leave the paper and come back in the other door.
...
between them matters. Hold on to those two demands. Everything in this chapter
is an attempt to meet exactly them and nothing more.
...
interior has two components, and the door you come in by decides where you can
get to.}
\label{fig:publications}
\end{figure}

\section{Two demands, and what the usual objects do with them}
...
\citep{ghoshal2009,battiston2020}. That is real progress and it is the whole of
the modern higher-order-network literature's first move. It meets the second
...
\citep{joslyn2017}, and it is worth stating their construction in the language
this book will use throughout. Begin by dividing the things in a system into
...
go into the same bag.

\section{The definition}
\label{sec:definition}

The remaining step is small, and once seen it is hard to unsee. If the trouble
...
Read slowly, that sentence is self-referential in the way the systems it
describes are self-referential, and that is the point rather than a stylistic
accident. Two properties are packed into it. \emph{Self-reference:} complexes
...
Two conventions are worth flagging now. First, quantities defined at the
...
plus its number of references. (As an aside that costs nothing: an author's
$h$-index is a statistic of the chy-degrees of the complexes that include that
atom. Bibliometrics is chygraph bookkeeping.)
...
it otherwise resembles --- is the whole of the difference between being inside
something and having something inside you.

\subsection{The two ways out of a complex}

Here is the sentence the rest of the book runs on, and it is worth stopping over
because every recursion in Chapters~\ref{ch:percolation} to~\ref{ch:overlap} is
a formalisation of it.
...
hypergraph, and this is where the second of our two demands earns its keep:
going down is not a matter of choosing one of $c$ members at random, because the
interior may be several disjoint pieces and you can only reach the piece you are
standing in. Enter a publication as one of its authors and you reach the other
authors; enter it as one of its references and you reach the other references.
The relevant number is therefore not the cardinality but the size of the
\emph{intra-complex component} you arrive in, which we will write $s$
(Fig.~\ref{fig:twoways}).
...
\node[lb,anchor=west,align=left] at (5.35,-1.30)
  {\emph{down:} into the interior ---\\ but only the component\\ you came in by};
\end{tikzpicture}
\end{adjustbox}
...
size $3$ and $2$; arriving as an author, three complexes are reachable, not
five. Percolation (Chapter~\ref{ch:percolation}) is this picture with a
probability attached to each arrow; the cavity recursion
...
as ``cardinality minus the one I arrived by'' and lose nothing.

\section{Layers}
\label{sec:layers}

One more piece of bookkeeping, and it is the piece that makes the whole
programme work computationally.
...
maximum in the definition is what settles the fight.

For the reader's peace of mind: no calculation in this book requires computing
$L(\chi)$ for a given chygraph. In practice one \emph{chooses} the layers when
...
Now the payoff, and it is the reason the construction is worth the definitions.
...
\paragraph{A clustered graph} --- and this is the mapping the second half of the
book lives on --- becomes a chygraph by \emph{promoting its dense motifs to
complexes}. A graph rich in triangles is given two complex layers, one of
...
what cardinalities they carry. This is the claim the rest of the book cashes:
...
The list is not a curiosity. It is the argument for the whole construction. The
...
These are the same object and it is worth seeing the two side by side once, in
Fig.~\ref{fig:twodrawings}, because readers who do not are liable to think a new
construction has been introduced somewhere around Chapter~\ref{ch:statmech}.

The inclusion drawing is honest and scales badly: it puts every complex and
every inclusion on the page, and it becomes unreadable at a couple of dozen
complexes. The spatial drawing is compact and slightly dishonest: it can only
show a complex as a box when the complex's members can be drawn adjacent, which
fails as soon as two complexes share more than one member --- precisely the
...
moments: $\ave{\kappa}$, $\ave{\kbar}$, $\ave{s}$, $\ave{\sbar}$. It is worth
registering how little that is. Whatever the sizes of the complexes, whatever
...
lifted in Sec.~\ref{ch:giant}. And moments are moments: two ensembles can agree
...
I close with the property that motivates everything after
Chapter~\ref{ch:percolation}, and with its price, because the price is
structural and no amount of care will make it go away.
...
come back. Real graphs are not treelike. Your friends know one another, and the
triangle they form is a loop of length three, the shortest there is.
...
clustered graphs and shows what recovers it.}
\label{fig:price}
\end{figure}

So the condition is easy to state and easy to check: \textbf{a chygraph is
locally treelike when its complexes meet in at most one atom}. Uniform
...
I have put this on the table in the chapter that defines the object, rather than
in a remark at the end of the book, because it is the honest place for it. The
next ten chapters develop what the construction buys, which is a great deal;
Chapter~\ref{ch:overlap} returns to Fig.~\ref{fig:price}b, measures the damage
on hyperbolic random graphs --- between $65$ and $83$ per cent of the answer
recovered --- and describes what would be needed to recover the rest. Clustering,
it will turn out, is not one obstruction but a hierarchy of them: promoting
