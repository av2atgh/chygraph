...
The objects this book runs its calculations on are not graphs, and this chapter
says what they are instead. The case for wanting something else is best made on
a system that the field has already mapped, and mapped repeatedly without ever
agreeing on the map: the scientific literature. Four standard drawings of it are
in print. All four are correct. No two of them are the same object, and the
reason is not that their authors disagreed about the data. It is that each of
them had to throw part of the system away before the drawing could begin.

\section{A paper is not a set of links}
\label{sec:paper}

This chapter defines the chygraph and shows what it represents that a graph
or a hypergraph cannot. The motivating example is the scientific literature,
which has been mapped many times over, whose maps disagree with one another, and
...
Call the paper one object, and its two lists the internal structure of that
object. Then the paper's interior is not a single group but two disjoint groups: a
...
not contain hyperedges. A paper cannot cite.

The first demand was met by Joslyn and Nowak, who defined \emph{ubergraphs}
\citep{joslyn2017}. Their construction is restated here in the language this
book uses throughout. Begin by dividing the things in a system into those the
modelling declines to decompose any further --- call them \emph{atoms} ---
...
here because people are not modelled as made of parts, and nothing
...
contain complexes. A paper can now cite --- and it still cannot tell its
...
just given to the exterior: make it a hypergraph.
...
happens to include no complexes, so $A$ need not be carried separately:
...
A price should be attached to the definition immediately, because the rest of
the book is an argument about that price. Nothing in either sentence bounds how
large a complex may be or how deep the nesting may run, and it is worth knowing
at once which of those two freedoms costs anything. The depth does not. The
threshold of every model in this book comes out of the determinant of a
$2L^{2}\times2L^{2}$ matrix assembled from four $L\times L$ matrices of first
moments, where $L$ is the number of \emph{layers} --- two for a graph, three for
the household model of Chapter~\ref{ch:epidemics} --- and the cardinalities of
the complexes do not enter its size at all. The width does. Summing the interior
of a complex exactly, which is what buys the whole trade, costs $2^{c}$ for a
complex of $c$ two-state members, and that is the only exponential anywhere in
the scheme. So a chygraph is cheap in the direction one expects it to be
expensive and expensive in one place only, which is also the place where the
loops went. Chapter~\ref{ch:metacomplex} is what happens when that one place
grows without bound.

Figure~\ref{fig:ladder} puts the three constructions side by side. Each step
adds exactly one property --- complexes inside complexes, then structure inside
a complex --- and a chygraph needs both.
...
A handful of words, each meaning exactly what it says.
...
They part company only for genuinely compound complexes like the publications
above.
...
the complexes are partitioned into \emph{layers}, indexed $l=0,\dots,L-1$, with the
...
simplicial complexes and for clustered graphs
\citep{battiston2020,bianconi2021}. If all of them
...
one object. Specifying an ensemble amounts
...
It is worth putting a real object underneath those four letters before they are
used for three hundred pages, and the publications chygraph of
Sec.~\ref{sec:paper} will do. Layer $0$ is the authors, layer $1$ the
publications. Then $\Phi^{0}_{1}$ is the generating function of an author's
productivity: an author with three papers contributes $x^{3}$ to it, and
$\Phi^{0\prime}_{1}(1)=\ave{\kappa}_{01}$ is the mean number of papers per
author. Going the other way, $G^{1}_{0}$ generates the size of the author list
of a publication \emph{entered through one of its authors}, so a four-author
paper contributes $y^{4}$, and $\ave{s}_{10}=G^{1\prime}_{0}(1)$ is the mean
author-list size. The excess functions are the same objects with the arrival
subtracted: $\bar G^{1}_{0}$ generates $s-1$ under the size-biased measure,
because a paper is reached by picking one of its authors and the author picked
is not one of the coauthors then found. An author picked at random has
$\ave{\kappa}_{01}$ papers; an author picked by walking in along a paper has
$\ave{\kbar}_{01}+1$ of them, and the difference between those two numbers is
the whole of what makes a branching process on a network different from one on a
tree of fixed degree.
...
Two caveats, stated here rather than later. Equation~\eqref{eq:moments}
...
\section{Treelike over complexes}
\label{sec:treelike}
\index{treelike|textbf}
\index{overlap!between complexes}

The chapter closes with the property that motivates everything after
...
become an interior. This is the trade the book explores, and Fig.~\ref{fig:price}a
is the good case.
...
is wrong on them; Sec.~\ref{sec:mergeerror} draws the object and prices the
...
\end{tikzpicture}
\end{adjustbox}
\caption{\textbf{The trade and its price.} (a) Two triangles, $a$ and $b$,
...
buys; Chapter~\ref{ch:cover} measures the damage on hyperbolic random graphs,
where the chygraph recovers between $77$ and $100$ per cent of the leaf-removal
core, and Chapter~\ref{ch:overlap} returns to Fig.~\ref{fig:price}b and
describes what would be needed to recover the rest. Clustering
