...
most one node. The measurements and the repair in this chapter are new;
the region-graph machinery they use is Kikuchi's, in the form given by
\citet{yedidia2005}. Chapter~\ref{ch:statmech} located it precisely, in the step where
...
Three questions follow. How much does the violation cost? What would repair it?
And why is the repair not simply the next chapter rather than an open problem?
The answers: less than one might fear but not nothing; a region graph with
messages on it; and because the repair
...
There is a repair more direct than a region graph, and it is the first thing
anyone proposes on seeing Fig.~\ref{fig:sharededge}: if two complexes share more than one atom, stop
...
discouraging and precise (Table~\ref{tab:mergecost} and
Fig.~\ref{fig:merge}).
...
atoms and does not move with $n$: the merging terminates, the repair is exact, and it is
...
feasibility of this chapter's repair is a Part~II calculation --- one can ask, of a
...
And on \emph{this} ensemble it terminates only in the very sparse régime. The
qualifier carries the weight, and Sec.~\ref{sec:placed} is about why:
hyperbolic random graphs are one ensemble of clustered graphs, the verdict
above is theirs, and on another ensemble at the same clustering coefficient it
comes out the other way. Merging and generalised belief propagation are the two
ends of one dial --- merge everything and the messages become trivial but the
interior impossible; merge nothing and the interior is easy but the messages
approximate --- and where to sit on it is an ensemble-by-ensemble question.
Figure~\ref{fig:merge} is how one answers it for a given ensemble; it is not a
verdict on clustered graphs, and Sec.~\ref{sec:whatwouldclose}'s route is what
remains where the answer is no.

\paragraph{A practical note.} Between those extremes lies a range where merging
works on the instances one actually has, rather than on an ensemble. Finding
which complexes to merge is a mechanical, checkable operation: build the
...
\section{An ensemble where it terminates}
\label{sec:placed}
\index{Karrer--Newman model|textbf}
\index{null model}

Section~\ref{sec:merging} measured the merge closure on hyperbolic random
graphs and found it affordable only when the graph is very sparse. Taken as a
statement about clustering that would be wrong, and Chapter~\ref{ch:chygraphs}
has already said why: a graph whose triangles are \emph{placed} rather than
grown satisfies the treelike condition, because the triangles are placed
independently. This section measures the difference, and it turns out to be the
whole of the difference.

\citet{karrer2010} give the ensemble to measure it on. One specifies, per
vertex, how many single edges it carries and how many copies of each chosen
subgraph; the edges are matched in pairs and each subgraph's corners in tuples,
exactly as a configuration model matches stubs. Clustering is then whatever the
motif density says it is, and it can be set as high as one likes. Two placed
motifs may share a vertex freely. They share an \emph{edge} only by
coincidence --- which is the property that makes their model solvable and, it
turns out, the property that decides Sec.~\ref{sec:merging}'s question.

\paragraph{The quantity that decides it is a count, not a fraction.} Both
...
\hline\hline
\end{tabular}
\caption{\textbf{The same clustering, opposite verdicts.} Maximal-clique pairs
...
\paragraph{An ensemble built out of overlaps.} The sharpest case is one
\citeauthor{karrer2010} raise themselves. Their edge-triangle model, they note,
is unrealistic in exactly the way that matters here --- real triangles share
edges and theirs do not --- and their remedy is to promote the shared-edge pair
to a subgraph type of its own: a diamond on four vertices, placed like any other
element. That is Sec.~\ref{sec:merging}'s merge, performed when the ensemble is
written down instead of afterwards.
...
is the same $O(1)$ residue, about a hundred elements at every size, which is a
fifth of them at the smallest and a thirtieth at the largest. The merge is not a
patch applied to a broken mapping. On this ensemble it inverts the modelling
decision that made the ensemble.

\paragraph{One caveat, and it is about finite size.} The $O(1)$ constant grows
...
they take over grows with the density. A measurement at one $n$ decides nothing
here; the slope against $n$ decides everything (Fig.~\ref{fig:motifs}).

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/fig-motifs.pdf}
\caption{\textbf{Flat against linear.} Left: maximal-clique pairs sharing two or
...
\paragraph{What replaces the claim this section removes.} Not that clustering is
harmless, and not that the merge always works. The condition is about where the
overlaps come from.
...
geometry or a growth rule producing whatever motifs it produces --- nobody has
chosen anything, the overlaps arrive at every scale, and their number is
extensive. Clustering is compatible with either, which is why
Chapter~\ref{ch:data} measures $\mathrm{shared}_{2+}$ on real data rather than
inferring it from the clustering coefficient.

The practical instruction is one line. Measure the offending-pair count at two
sizes: if it is flat, merge; if it grows like $n$, do not, and
...
above. This is what the cluster variation method has always been, and the chygraph is one rung of it made
...
Until that is done the position is the one this chapter opened with. The
chygraph is exact when complexes meet in at
...
The measurements are new here: what the two countings cost on overlapping
complexes, what running the messages recovers over sixty instances, where
merging stops being affordable, and Sec.~\ref{sec:placed}'s contrast between a
placed and a geometric ensemble. The first is checked against arithmetic. The
two-triangle table compares three schemes with an exact sum over sixteen
configurations, and the counting numbers are checked for what they are supposed
to guarantee: every node covered once by both schemes, every \emph{factor}
covered once by M\"obius and twice by Bethe on the shared bond. Where the
iteration converges, the consistency condition
$\sum_{x_{P}\setminus x_{R}}b_{P}=b_{R}$ holds to $2\times10^{-6}$, so what
remains is the approximation's error and not the solver's.

\checkhead{Partial results}
Six of the twenty non-chordal instances do not converge at any damping tried up
to $0.999$, and no number from them is quoted; the convergence criterion is
stated rather than implied, the residuals falling into two clusters separated by
four orders of magnitude with the threshold in the gap. Generalised belief
propagation is not variationally bounded, so on a badly built region graph it
can be worse than the static estimate. Section~\ref{sec:placed}'s $O(1)$ overlap
count is an estimate supported by a measurement flat over an eightfold range in
$n$, not a theorem: it is not proved that the count is bounded for every motif
set, and at high density the finite-size crossover is late enough to invert the
answer at $n=4000$. The two ensembles compared there are matched on the
clustering coefficient and on nothing else, which is the comparison the section
needs but leaves the degree distributions unmatched. And the whole chapter is
instance-level:
the ensemble version of Sec.~\ref{sec:whatwouldclose} does not exist, and
