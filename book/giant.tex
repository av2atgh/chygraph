...
Chapter~\ref{ch:percolation} ended with a threshold and left the reader with a
question it did not answer. The
results of this chapter are reported here for the first time; where they extend
the percolation map of Refs.~\citep{vazquez2023pre,vazquez2024comnet} the debt
is marked at the point of use. The tensor $A$ is built from four matrices of first
moments; the map, on the other hand, needs whole generating functions. So the
threshold uses strictly less information than the map. What is the rest of it
buying?

The answer is a hierarchy, Table~\ref{tab:hierarchy}, and it is unusually clean:
each additional order of moment data buys exactly one more thing about the
transition.
...
degeneracy is the point, so here it is.
...
\code{figs/beyond\_threshold.py}.}
...
\begin{calculation}{an arithmetic slip that reinstating the distribution exposed}
Reference~\citep{vazquez2024comnet} gives the mean intra-complex component size
...
parameter of the triangle layer, $S_{2}$.
\end{calculation}
...
\begin{calculation}{the amplitude}
...
unknowns one would have written down by hand.
\end{calculation}

Table~\ref{tab:amplitudes} collects the closed forms on the critical manifold.
Two closed forms. For a graph with an arbitrary degree distribution,
sites present with probability $p$ and bonds with probability $q$,
\begin{equation}
...
breakdown, which is the best one can ask of it --- but the closed forms of
Table~\ref{tab:amplitudes} are statements about ensembles with finite second
...
transitions. For anything that maps to a chygraph this now has a one-line proof:
a complex contributes to $C$ through second derivatives of its generating
functions, and those cannot be negative.

One caveat delimits the scope, and it is a real exclusion rather than a
...
Now the assumption. Section~\ref{sec:ensembles} took the generating functions to
factorise over target layers, $\Phi^{l}(x)=\prod_{k}\Phi^{l}_{k}(x_{k})$. That
...
degree distributions of exactly this kind are what the motif formalism of Mann
\emph{et al.} \citep{mann2021} works with, and Chapter~\ref{ch:data} recommended
...
Eq.~\eqref{eq:qc3}. The curves cross near $q=0.42$, so no single statement about
...
Bianconi and Dorogovtsev \citep{bianconi2024} distinguish two rules for what a
...
rule for a chemical reaction, a catalytic step, a supply chain. Ha, Neri and
Annibale \citep{ha2025} reach the same distinction independently.
...
reported in Ref.~\citep{bianconi2024}. For Poisson degrees and cardinalities,
...
Cirigliano \citep{cirigliano2025} solves site percolation on Static Triadic
...
Ref.~\citep{cirigliano2025}'s; the exact value is recovered below,
...
\begin{equation}
  ...
  \label{eq:stcthresh}
\end{equation}
with $b=\ave{k(k-1)}/\ave{k}$, which is Cirigliano's exact result, and an order
...
The moral is the one this book keeps arriving at from different directions, and
...
every mapping in Chapter~\ref{ch:percolation}, by comparing the Jacobian of the
map against an independent implementation of the published tensor; the Perron
root reproduces the published pseudo-order parameter through
$\lambda^{2}=\theta+1$. The closed-form amplitudes are checked against the
numerically solved map, and against an independent evaluation by numerical
eigenvectors on the full unreduced index set, which also confirms that the core
reduction is exact. The joint construction is checked in both directions: with
factorised inputs it reproduces the factorised map to machine precision, and
with correlated inputs against configuration-model simulation over all three
...
Every number quoted in this chapter is returned by the software, and the figures
are generated by running it.
