...
Two networks can share a percolation threshold to the last digit and behave
quite differently on the far side of it. In one the giant component creeps up
out of zero; in the other it arrives with a jump, and a system that was
disconnected at one density is macroscopically connected at the next. A
threshold is a single number, and a single number cannot tell those apart. So
what can, and how much more does one have to know about the ensemble before it
does?

That is this chapter's question, and the answer turns out to be unusually tidy:
each further order of moment data buys exactly one more feature of the
transition, and nothing short of the full distributions buys the last one.
...
strictly less information than the map, and this chapter asks what the
...
the map at its trivial fixed point. Consequently, $-\mathrm{vec}^{2}A=J-I$ has
the eigenvalues of $J$ each less one, and taking the largest of them,
\begin{equation}
  \Lambda=\lambda-1,
...
The observation is small: threshold and order
...
unknowns one would have written down by hand. \code{Chygraph.\allowbreak amplitude} carries this --- the directional
derivative, the core reduction and $B_{l}$ --- over
\code{amplitude.\allowbreak CriticalAmplitude}.
...
\]
The Perron root here is $\lambda=\sqrt{pq\ave{\bar k}}$, a square root because
the map alternates between the two directions of Sec.~\ref{sec:threshold} --- a
graph is one layer, but $J$ still has period two, and the root of a period-two
matrix is the square root of its two-step product. So
$pq\ave{\bar k}=(1+\Lambda)^{2}$ and the bracket is $2\Lambda$ to leading
...
Eqs.~\eqref{eq:Bgraph} and~\eqref{eq:Bhyper}. Every $\lambda$ is a square root
for the reason given below Eq.~\eqref{eq:Bgraph}: the map alternates between the
two directions of Sec.~\ref{sec:threshold}, so $J$ has period two whatever the
structure, and its Perron root is the square root of the two-step product. The
amplitudes then say what the thresholds cannot. Each denominator is one plus a
term counting how much second-moment weight the interior carries, so putting
connectivity into larger complexes makes the giant component rise more slowly
out of the threshold even where it lowers the threshold itself. The last row is
the sharpest case: $\ave{k}_{\triangle}$ appears and $\ave{k}_{|}$ does not, so a
link layer moves the transition without touching the slope on the far side of
it.}
...
This section lifts the factorisation assumption.
...
joint one. Differentiating a generating function is what produces a size-biased
count: $\sum_{n}n\,p_{n}x^{n-1}=\Phi'(x)$, and dividing by $\Phi'(1)=\ave{n}$
normalises it, which is the scalar $\bar\Phi=\Phi'/\Phi'(1)$ of
Sec.~\ref{sec:ensembles}. Differentiating with respect to $x_{m}$ alone
size-biases in layer $m$ and leaves the others as they are, which is exactly
what arriving through a layer-$m$ inclusion does. So
\begin{equation}
...
Take classes $c=2$ and $c=5$, and a one-parameter family in which both marginal
...
The tensor returns the threshold, and so does a count, which is worth doing
because it shows where the quadratic comes from. Follow the exploration from an
occupied node reached along a backbone edge: it has $b=\ave{k(k-1)}/\ave{k}$
further edges. Each ends at an occupied node, probability $p$, and that is one
new node; or at an unoccupied one, probability $1-p$, and the range-$2$ rule
then allows one further edge to $b$ more nodes, of which a fraction $p$ are
occupied. So one occupied node begets $bp+b(1-p)\,bp$ others, and the
transition is where that passes one. Writing it out,
\begin{equation}
  \begin{aligned}
