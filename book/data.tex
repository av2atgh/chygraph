\chapter{Where chygraphs come from}
\label{ch:data}

Chapter~\ref{ch:chygraphs} defined an object. This chapter asks where one gets
hold of it. There are exactly two routes, they behave very differently, and
which one you are on decides in advance how well the theory of the next nine
chapters will work on your data --- so it is worth knowing before any of that
theory is developed rather than after.
...
data. The two kinds of experiment therefore hand you different objects, and
...
\emph{What is an atom.} Whatever you have decided not to decompose. This is a
...
participation in one layer tells you nothing about its participation in another,
...
One rule of thumb is worth stating in bold, because Sec.~\ref{sec:promoting}
will supply the evidence for it: \textbf{if you have the group data, use it. Do
not flatten it to a graph and then try to recover the groups.} The flattening
is not invertible, and the object you get back is worse than either the graph or
the groups.

\section{Promoting the cliques of a real graph}
\label{sec:promoting}

Now the other route. You have a graph and want a chygraph. The recipe of
...
more nodes. Call it $\mathrm{shared}_{2+}$. It is zero for a treelike chygraph
...
Three things come out of it, and the third was not what I expected.
...
$1.01$ and $1.26$ --- within a quarter of the clustering-free reference --- and
at $\mathrm{shared}_{2+}$ between $0.000$ and $0.022$. Fewer than one
intersecting clique pair in forty-five shares an edge. For these networks the
chygraph of maximal cliques is treelike for practical purposes, and the theory
of the next nine chapters applies to them essentially without correction. That
is a better result than the framing of Sec.~\ref{sec:treelike} would have led
one to expect, and it is worth saying so plainly: the obstruction that
Chapter~\ref{ch:overlap} is about is not a generic obstruction. It is specific,
and one can tell in advance whether one has it.

\paragraph{One network fails completely, and it is the informative one.} The
...
Promotion has failed, not marginally but categorically.

The reason is the point of this chapter. Collins \emph{et al.} measured
...
\paragraph{The control is not always the cleaner object.} I had assumed that
destroying the clustering could only reduce the overlap, so that the
...
much its degree sequence manufactures, and that quantity grows with the excess
degree $\ave{k^{2}}/\ave{k}$. The Figeys network is the most heterogeneous of
...
wins on Stelzl ($C=0.006$), PDZ ($C=0.007$), Figeys ($C=0.040$) and, by a hair,
...
checked how much clustering the control has. Section~\ref{sec:hrg} is careful
...
for hyperbolic random graphs and their degree-matched controls. The paired ratio
is taken at identical degree sequence and seed, so $P(k)$ and assortativity are
held fixed and clustering is the only difference. $\theta$ is used for
structural exponents throughout, $\beta$ being reserved for the inverse
temperature.}
\label{tab:ensemble}
\end{table}
...
mean degree $2$, $4$ and $8$, flat to three decimals across the same range, and
the control above $\tau=3$ returns to that value. Those are the reference points
that make the ratio meaningful.
...
One check that the measurement is reading the right quantity. The number of
maximal cliques in a hyperbolic random graph should scale as $n^{(3-\tau)/2}$
\citep{friedrich2015}, which is $0.45$, $0.25$ and $0.05$ at $\tau=2.1$, $2.5$
and $2.9$. Measured, it is $n^{0.37}$, $n^{0.20}$ and $n^{0.07}$: consistently a
little below theory, as one expects at finite $n$, and tracking it across a
ninefold range in the exponent.
...
requires no theory at all, and it says how much of the answer the next nine
chapters can be expected to get.

At $\mathrm{shared}_{2+}\lesssim0.02$ --- eight of the ten networks of
...
Chapter~\ref{ch:overlap} measures the chygraph recovering between $65$ and $83$
...
The last thing this chapter owes the rest of the book is a way of generating
chygraphs to order, since every analytical result in
...
as the chygraph grows, which is exactly the regime the theory assumes.

That last clause is worth noticing, because it is the reason simulation
agreement is a weaker test than it looks. A random chygraph built this way
...
is what the hyperbolic random graphs of Sec.~\ref{sec:hrg} and the rewired
control of Chapter~\ref{ch:cover} are for. Both kinds of check appear throughout
the book and they answer different questions; I have tried to say which is which
at each point.
...
the argument that it would be worth doing: the one dataset here that began as
