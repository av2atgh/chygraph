...
Chapter~\ref{ch:chygraphs} defined an object. This chapter asks where to
...
what is left after the grouping has been thrown away. A purification recovers a
set of proteins together and reports no pairs at all; the pairs in the resulting
network were manufactured afterwards by someone deciding that every protein in
the set interacts with every other. A reaction consumes its substrates and emits
its products, and the distinction between the two is a partition of the
complex's interior that no pairwise drawing can hold. A dependency set is worse
still, because it carries a logic: an activity waiting on \emph{all} of its
predecessors and one waiting on \emph{any} of them project onto the same links,
and Sec.~\ref{sec:andor} shows the two percolate at thresholds a factor of
$3.5$ apart. In every row the grouping is the measurement and the graph is the
residue.}
\label{tab:systems}
\end{table}
...
Table~\ref{tab:real} and Fig.~\ref{fig:realoverlap} report both for ten real
...
\code{figs/\allowbreak real\_\allowbreak chygraphs.\allowbreak py}.}
\label{tab:real}
\end{table}
...
Fig.~\ref{fig:tworoutes}, and somebody flattened it onto route (b). Trying to
recover the groups by clique promotion is an attempt to invert that flattening,
and Fig.~\ref{fig:realoverlap}b shows what comes back: a cardinality
...
Fig.~\ref{fig:realoverlap}a. In four of the ten it sits \emph{above}. For the
...
every degree-matched comparison in this book, including the one in
Sec.~\ref{sec:hrg} that the whole programme rests on. \emph{Degree-matched}
...
Chapter~\ref{ch:cover} measures the chygraph recovering between $77$ and $100$
...
The construction is the chygraph version of the configuration model, and like
that model it works by matching stubs. Every complex carries two kinds of loose
end: an \emph{upward} stub for each complex that is going to include it, and a
\emph{downward} stub for each member it is going to have. Nothing else is
specified, and the ensemble is whatever results from joining the two kinds
blindly.

\begin{model}{Configuration model for chygraphs}
\begin{rules}
\rl{1}{Sizes} Fix the number $N_{l}$ of layer-$l$ complexes, for each layer
$l=0,\dots,L-1$.
\rl{2}{Chy-degrees} For every layer-$l$ complex and every layer $k$, draw the
number of layer-$k$ complexes that will include it from the distribution
generated by $\Phi^{l}_{k}$, and give the complex that many upward stubs.
\rl{3}{Cardinalities} For every layer-$l$ complex and every layer $k$, draw the
number of layer-$k$ members it will have from the distribution generated by
$G^{l}_{k}$, and give it that many downward stubs.
\rl{4}{Matching} For each ordered pair of layers $(l,k)$, pair the upward stubs
of layer $l$ with the downward stubs of layer $k$ uniformly at random, discarding
a pairing that would put the same member into a complex twice.
\end{rules}
\end{model}

What the rules produce is a random chygraph with the prescribed first-moment
matrices $\ave{\kappa}$, $\ave{\kbar}$, $\ave{s}$ and $\ave{\sbar}$ of
Sec.~\ref{sec:ensembles} and no further structure. In particular they produce a
treelike one. Two complexes of the same layer share a second atom only when two
of the random pairings in rule~4 happen to fall on the same pair of complexes,
and that is the event the ordinary configuration model counts as a multi-edge:
its expected number stays $O(1)$ however large the object grows, while the number
of complexes grows like $N$, so the fraction of intersecting pairs that share two
or more atoms is $O(1/N)$ and vanishes. That is exactly the r\'egime the theory
assumes --- and, read against Table~\ref{tab:real}, exactly the r\'egime the
Collins dataset at $\mathrm{shared}_{2+}=0.903$ is not in.
...
Fig.~\ref{fig:publications} is drawn and not measured; so are the reaction and
