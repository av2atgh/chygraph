...
\caption{\textbf{Fraction of nodes in the core.} The same
...
\caption{\textbf{Phase transition in the Karrer--Newman ensemble.} Fraction of atoms surviving
...
The finite-size evidence is worth stating exactly, because
Table~\ref{tab:knfinite} shows this part is not immune to reading a crossover as
a transition. Below the threshold the core \emph{falls} with $n$, from $0.0067$
...
contained, and by one that leaves the formalism where it found it --- a merged
complex is a complex, and a merged chygraph is a chygraph.

\paragraph{And it works for a reason that can be stated.} The chygraph
recursion is exact when the incidence structure of its complexes is a forest.
...
\paragraph{Where it does not, there is a phase transition behind it.} All $40$
cyclic families are Karrer--Newman, and Sec.~\ref{sec:coretransition} says why
in terms of the ensemble's own parameters rather than the outcome. Following an
...
That transition is what explains the instances of Sec.~\ref{sec:mergeerror}
rather than merely restating them. Those were drawn at $b$ between $3.5$ and
$7$, far above threshold, so two thirds of them keeping a core is where they
were sampled and not a peculiarity of the ensemble. Any ensemble put there would
do the same, and the hyperbolic and real neighbourhoods --- whose closures leave
one to three complexes --- are on the other side of it.
...
proved. The finite-size evidence is stated rather than assumed --- the core
