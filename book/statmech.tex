...
recursion at two values of a parameter, and checked it at both values. That is
...
it. The bookkeeping is worth doing carefully in one place: the same two steps,
...
Two sanity checks come for free and are worth taking, because they say exactly
...
Equation~\eqref{eq:up} is also where the treelike assumption sits, and it is
worth noticing now rather than in Sec.~\ref{sec:assumes}. Adding the $u_{b\to
...
and carries weight one. Replacing $\ave{u'}$ by $\ave{u'^{2}}$ throughout gives
...
three for everything in this book. That is the payoff of doing the interior
sum locally: it is expensive, and its expense never propagates.
...
ensembles have $T_{c}=8.4256$ and $6.8801$. The whole difference between the two
natural null models is one term that is easy to drop.
...
\paragraph{One diagnostic does not survive the move.} It is worth naming,
because it is the natural thing to reach for and it is wrong. The sign of the
...
is also where the diagnostic of Sec.~\ref{sec:fixedpoint} earns its place.
