...
function $\hat\nu(\sigma_{0})\propto Z(\sigma_{0})$. A function of one Ising
...
$\hat\nu(\sigma_{0})\propto e^{u\sigma_{0}}$, fixes $e^{u}/e^{-u}=Z(+1)/Z(-1)$,
...
The general Bethe free energy is one term per complex, one per node, and one
subtracted per inclusion,
\[
...
energy with respect to the external field $\mu$ of Eq.~\eqref{eq:up} gives the
...
usual:
\begin{equation}
  m=\frac{1}{N}\frac{\partial\ln Z}{\partial(\beta\mu)}
   =-\frac{\partial f}{\partial\mu},
  \qquad
  \chi=\frac{\partial m}{\partial\mu}
...
emits, the external field contributes $e^{\mu\sigma_{i}}$, and
\begin{equation}
...
is stationary in the messages, differentiating Eq.~\eqref{eq:bethe} in $\mu$
picks up only this explicit dependence, and Eq.~\eqref{eq:mfromZ} gives
$m_{i}=\partial\ln Z_{i}/\partial\mu=\tanh H_{i}$.
...
\begin{equation}
  R^{l}=\delta_{\mu}\ast\Bigl(\ast_{k}\,\Phi^{l}_{k}\!\ast\,Q^{lk}\Bigr),
  \qquad
  m_{l}=\int\tanh(H)\,\mathrm{d}R^{l}(H).
  \label{eq:orderparam}
\end{equation}
The order parameter is one more convolution and not a new object. Everything
below is Eq.~\eqref{eq:orderparam} evaluated: at $\mu=0$ below the transition,
and differentiated once more at $\mu\to0$ above it.

\paragraph{The response.} Above the transition every message is $O(\mu)$, so
the down step is its linearisation Eq.~\eqref{eq:uprime} and the up step
Eq.~\eqref{eq:up} adds the field at the complex. Write $\hat\chi_{m}$ for the
mean cavity field sent into a layer-$m$ complex, divided by $\mu$. A complex
reached through a layer-$m$ inclusion still belongs to $\ave{\kbar}_{m}$ further
layer-$m$ complexes and $\ave{\kappa}_{l}$ layer-$l$ complexes for $l\ne m$,
each of which delivers $\ave{\sbar u'}_{l}$ per unit field --- which is
...
\begin{equation}
  h_{m}=\mu+\sum_{l}\bigl(\kappa_{l}-\delta_{lm}\bigr)\,\bar u_{l}(h_{l}),
  \qquad
  m=\tanh\Bigl(\mu+\sum_{l}\kappa_{l}\,\bar u_{l}(h_{l})\Bigr),
  \label{eq:magclosure}
\end{equation}
$L$ scalars and no population. The Kronecker delta is the excess bracket of
...
so $\bar u_{l}(h)=a^{(l)}_{1}h+a^{(l)}_{3}h^{3}+O(h^{5})$, and both
...
entered member held at $+1$, spin-flip symmetry makes $Z(-1)$ at $h$ equal
$Z(+1)$ at $-h$, so with $g$ the cumulant generating function of $S$ the emitted
