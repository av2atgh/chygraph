...
\paragraph{Replica symmetry, and Eq.~\eqref{eq:bethe}.} The
replica-symmetric ansatz is that a replica state is a superposition of product
states weighted by a field,
\begin{equation}
  \rho_{l}(\vec\sigma)=\!\int\!\mathrm{d}h\,P_{l}(h)
    \prod_{r}\frac{e^{h\sigma^{r}}}{2\cosh h},
  \qquad
  \hat\rho_{l}(\vec\sigma)=\!\int\!\mathrm{d}u\,Q_{l}(u)
    \prod_{r}\frac{e^{u\sigma^{r}}}{2\cosh u},
  \label{eq:rsansatz}
\end{equation}
which is where a field first enters the calculation --- and it enters as an
...
\begin{equation}
  \rho_{l}(\vec\sigma)=\int\!\mathcal{D}Q\ \mathcal{P}_{l}[Q]
    \prod_{\text{blocks}}\Bigl[\int\!\mathrm{d}h\,Q(h)
      \prod_{r\in\text{block}}\frac{e^{h\sigma^{r}}}{2\cosh h}\Bigr],
  \label{eq:rsbansatz}
\end{equation}
