...
\section{One condition, every structure}
...
Eq.~\eqref{eq:emitted} to supply (Table~\ref{tab:structures}). The five entries
of Table~\ref{tab:structures} differ in how many onward branches a complex
offers and in nothing else: the structure enters through a counting factor, the
model enters through $u'$, and the two never mix.
...
which atoms each complex holds.

Nobody knows how to do that average directly, because the logarithm does not
come apart over the complexes the disorder lives on. So the calculation goes
around it by Eq.~\eqref{eq:replicatrick}, the replica trick, $\overline{\ln
Z}=\lim_{n\to0}(\overline{Z^{n}}-1)/n$. Everything below is done at positive
integer $n$, where $\overline{Z^{n}}$ is an average over $n$ independent copies
and does factorise, and the continuation to $n\to0$ is taken at the end without
being justified --- which is the standing caveat of
Sec.~\ref{sec:replicas} and is not repaired here. The quantity actually computed
is therefore
\begin{equation}
