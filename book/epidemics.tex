...
This chapter is the payoff of the two before it, and it is the one place in the
book where the reader is entitled to ask what any of it is for. An epidemic is
...
Sections~\ref{sec:sirperc}--\ref{sec:rstar} do that, and they are exposition of
solved work. Section~\ref{sec:groupcontagion} is the step past it. If catching
...
would have to. That is where an outbreak can become explosive, and it is
Chapter~\ref{ch:ising}'s unanimity interaction in different clothes.
...
\end{equation}
the \emph{transmissibility}, and --- this is Newman's observation
\citep{newman2002} --- if $\tau$ is the same for everyone, the outcome on each
...
outside, at work, on transport, at school. Ball, Mollison and Scalia-Tomba
\citep{ball1997,ball2010} built the standard model of this --- \emph{two levels
...
that single zero is the whole of what is different in the input.
...
Sec.~\ref{sec:ensembles}. It is not an analogy. The two are the same
...
$\mu_{H}=\bar s_{\triangle}(p_{H})=2p_{H}(1+p_{H}-p_{H}^{2})$. This is the
kind of coincidence the whole book is arranged to produce.
\end{calculation}
...
Now substitute into the threshold tensor of Sec.~\ref{sec:threshold} and turn
the handle. With a Poisson global contact network of mean degree $\ave{k}$ and
global transmissibility $T$, the result is
\begin{equation}
...
Ref.~\citep{ball1997}. It has a plain reading, and the reading is the same
...
It is worth being clear about what has and has not happened here. The
epidemiological literature derives $R^{\ast}$ by constructing a branching
...
the same chygraph without further input (Fig.~\ref{fig:householdfig}). For
households of size three on a Poisson global network with $\ave{k}=2$,
\begin{center}\small
\begin{tabular}{lccc}
\hline\hline
...
\end{tabular}
\end{center}
Households do two separable things, and it is worth naming them separately.
They lower the threshold, by a factor of three here. And they steepen the
...
may need reinforcement --- two of your three housemates, not one
...
This looks like a small change and it is not. It takes the problem out of
Chapter~\ref{ch:percolation} entirely, and the reason is structural rather than
technical.

\paragraph{Why percolation cannot do it.} Equations~\eqref{eq:Qminus}
and~\eqref{eq:Qplus} are a product over onward routes. That product is the
mathematical form of an assumption: each route succeeds or fails on its own, so
the state travelling along an inclusion can be one number. Under a threshold
rule a group's behaviour depends on \emph{how many} of its members are infected
at once, and there is no single number per inclusion that carries that. What has
been lost is not accuracy but the class of problem.

The consequence is visible without any calculation. With $r\ge2$ a single
infected seed cannot activate any group, so it infects nobody, so $Q\equiv1$ ---
nothing connected to anything --- is not merely a fixed point but a
\emph{stable} one, at every parameter value. Section~\ref{sec:breakdown} named
this exclusion when it arose for interdependent networks: the transition, if
there is one, happens by a saddle-node bifurcation while the trivial fixed
point is still stable, so $\Lambda=0$ does not locate it. The same sentence
covers threshold contagion. Section~\ref{sec:breakdown}'s proof that group
structure alone gives continuous transitions is untouched by this and is worth
restating precisely, because the two are easily confused: within the percolation
class every group interaction contributes non-negatively to the curvature $C$
and the onset is continuous; a threshold rule is not in the class.

\paragraph{What replaces it.} Since the tensor cannot be used, the honest move
is to write the simplest self-consistent equation that keeps the mechanism and
solve it. Take a population with two kinds of connection: Poisson-many pairwise
contacts, mean $\ave{k}_{|}$, each transmitting with probability $T$; and
Poisson-many three-member groups, mean $\ave{k}_{\triangle}$, each transmitting
to a member with probability $T_{\triangle}$ but only when \emph{both} others
are infected. Write
\begin{equation}
  \beta_{1}=T\ave{k}_{|},
  \qquad
...
\end{equation}
For Poisson degrees the excess distribution equals the ordinary one, so the
cavity probability that a neighbour is infected is the population fraction
$\rho$ itself, and the tree equation and the mean-field equation coincide. An
individual escapes if no pairwise contact transmits and no group activates:
\begin{equation}
  \rho=1-e^{-\beta_{1}\rho-\beta_{2}\rho^{2}}.
  \label{eq:contagion}
\end{equation}
The $\rho^{2}$ is the whole of the higher-order physics --- two infected others
...
is a statement worth having in a public-health argument and one that no
threshold calculation would ever produce.
...
the simplicial Ising model of Ref.~\citep{son2026}, whose hyperedges lower the
...
energies. It is the same physics, and the reader who has followed this section
has already met its difficulty: a rule that needs several participants at once
cannot be carried by one number per inclusion.
...
otherwise for anything that maps to a chygraph. If several are required, the
outbreak can appear at full size or not at all, cannot be started by a single
seed, and persists below the coupling at which it could have begun. Which regime
...
connectivity recursion. Equation~\eqref{eq:Kn} at $n=3$ is checked term by term
...
households; the amplitudes are obtained by numerical eigenvectors on the full
index set, independently of the symbolic route of Sec.~\ref{sec:amplitude}.

For Sec.~\ref{sec:groupcontagion} the tricritical condition
Eq.~\eqref{eq:tricritical} is checked in two ways: symbolically, that the
quadratic coefficient at $\beta_{1}=1$ is $\beta_{2}-\tfrac12$; and
numerically, that the fold lies below $\beta_{1}=1$ exactly when
$\beta_{2}>\tfrac12$. The exponent $\beta=\tfrac12$ at the tricritical point is
confirmed by solving Eq.~\eqref{eq:contagion} at three decades of
$\beta_{1}-1$.

Equation~\eqref{eq:contagion} is not a chygraph calculation and is not offered
as one. It is a mean-field equation, exact on a tree with Poisson degrees and
nothing more, and its role in this chapter is to establish that the qualitative
change is real --- not to be the theory of threshold contagion. What that theory
would need is a message that carries more than one number per inclusion, which
is what the whole of Part~III is about.
