...
A disease passes from an infected person to someone they meet. A worm passes
from an infected machine to another one it can reach. A rumour, a fashion, a
piece of misinformation, an idea: each passes from someone who holds it to
someone they talk to. The particulars could hardly be less alike --- a virion,
a packet, a sentence --- and neither could the timescales, from the years of a
pandemic to the minutes in which a worm takes an entire network. What the four
share is a shape. There is a population of holders and non-holders; there are
contacts along which the thing can pass and pairs between which it cannot; each
contact either carries it or does not; and the question asked of all of them is
the same one. Does the thing die out among a handful of early holders, or does
it reach a finite fraction of everybody?

That question does not need the particulars. Decide in advance, for each
contact, whether a passing along it would have succeeded, and keep the contacts
that would have carried it. The set of people, or machines, reached from a
given starting point is then the connected component containing it, and the
question above has become the question of whether a giant component exists. So
the threshold separating a fizzle from an outbreak, and the fraction of the
population an outbreak reaches, are percolation quantities, and they can be had
without knowing anything about virology, or protocols, or why one rumour
travels and another does not. What is lost in the trade is the time course:
percolation says who is reached in the end and never says when.
...
else; and the household reproduction number of mathematical epidemiology is
calculated from Chapter~\ref{ch:percolation}'s threshold tensor with no
epidemiology supplied.
...
would have to. That is where an outbreak can become explosive.

\section{Mapping to percolation}
\label{sec:sirperc}
...
\begin{model}{Two levels of mixing, as a chygraph}
\begin{rules}
\rl{1}{Individuals} Layer $0$ is the individuals, one atom each.
\rl{2}{Households} Layer $1$ is the households: one complex per household, whose
intra-complex graph is the complete graph on its members, so that inside a
household everybody meets everybody. Each individual is included in exactly one
of these.
\rl{3}{Global contacts} Layer $2$ is the contacts made outside the household:
one complex per contact, holding the two individuals it joins.
\end{rules}
\end{model}

What the three rules produce is the standard model of two levels of mixing with
no epidemiology written into it anywhere --- no infectious period, no rate, no
household size distribution beyond the one in $G^{1}_{0}$. Everything the model
is usually specified with has been pushed into the two occupation probabilities
$p_{H}$ and $T$ and into the layer data, which is what makes it a substitution
into Chapter~\ref{ch:percolation} rather than a fresh calculation.
...
The within-household outbreak is a percolated clique, and the classical final-size distribution is what the intra-complex generating function has to return.
...
$\mu_{H}=\bar s_{\triangle}(p_{H})=2p_{H}(1+p_{H}-p_{H}^{2})$. \code{applications.\allowbreak clique\_\allowbreak excess\_\allowbreak pgf} is the recursion and the size-biasing together.
...
\code{figs/\allowbreak epidemics.\allowbreak py}.}
...
minimum of that function. \code{figs/\allowbreak epidemics.\allowbreak py} solves Eq.~\eqref{eq:contagion} for $\beta_{1}$ rather than for $\rho$, so the fold is a minimum and no root-finding is needed.

Figure~\ref{fig:contagion} draws the calculation. One dial, $\beta_{2}$,
...
Equation~\eqref{eq:tricritical} places the boundary at $\beta_{2}=1/2$. Generated by
\code{figs/\allowbreak epidemics.\allowbreak py}.}
\label{fig:contagion}
\end{figure}
...
\emph{Say what is held fixed.} Section~\ref{sec:rstar}'s table holds the global
...
$T_{c}=0.2500$ to $0.2929$, and at $T=0.3$ the outbreak reaches $5.6$ per cent of the
population with households against $31.4$ per cent without. Household contacts are
