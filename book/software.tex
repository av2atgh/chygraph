...
Fig.~\ref{fig:hrgc} and the clustering of those same graphs, the core sweep
along the correlation axis behind Figs.~\ref{fig:vw} and~\ref{fig:vwt} and the
clustering scan quoted beside it, the sixteen real networks of
Table~\ref{tab:realcore}, and the seven measurements of Part~IV --- the cavity method on
...
Fig.~\ref{fig:coretransition}. Those fourteen are the only places
...
\begin{longtable}{@{}L{0.085\textwidth} L{0.235\textwidth} L{0.36\textwidth} L{0.18\textwidth}@{}}
...
\eqref{eq:branchcond} & one layer, both thresholds & \code{figs/\allowbreak potts.\allowbreak py:\allowbreak check\allowbreak \_\allowbreak thresholds} & same file\\
...
\eqref{eq:magclosure} & the magnetisation, regular chygraph & \code{api.\allowbreak Chygraph.\allowbreak magnetisation} & \code{test\_\allowbreak ising}\\
\eqref{eq:a1a3} & the interior cumulants $a_{1}$, $a_{3}$ & \code{ising.\allowbreak interior\_\allowbreak cumulants} & \code{test\_\allowbreak ising}\\
\eqref{eq:magamp} & its critical amplitude & \code{api.\allowbreak Chygraph.\allowbreak magnetisation\_\allowbreak amplitude} & \code{test\_\allowbreak ising}\\
\eqref{eq:chi} & the susceptibility & \code{api.\allowbreak Chygraph.\allowbreak susceptibility} & \code{test\_\allowbreak ising}\\
\eqref{eq:chiamp} & its critical amplitude & \code{api.\allowbreak Chygraph.\allowbreak susceptibility\_\allowbreak amplitude} & \code{test\_\allowbreak ising}\\
\eqref{eq:sigmagen} & the complexity at $m=0$ & \code{figs/\allowbreak colouring.\allowbreak py:\allowbreak \_\allowbreak sv\_\allowbreak sigma} & same file\\
...
\eqref{eq:a3edge} & $a_{3}$ for a bond & \code{ising.\allowbreak interior\_\allowbreak cumulants} & \code{test\_\allowbreak ising}\\
\eqref{eq:chibethe} & the Bethe-lattice $\chi$ & \code{api.\allowbreak Chygraph.\allowbreak susceptibility} & \code{test\_\allowbreak ising}\\
\eqref{eq:magampgraph} & the graph amplitude & \code{api.\allowbreak Chygraph.\allowbreak magnetisation\_\allowbreak amplitude} & \code{test\_\allowbreak ising}\\
...
\eqref{eq:tcmulti} & $T_{c}$ over several layers & \code{ising.\allowbreak branching\_\allowbreak matrix} & \code{figs/\allowbreak ising.\allowbreak py}\\
\ref{tab:fiveising} & the five structures at one degree & \code{figs/\allowbreak ising.\allowbreak py:\allowbreak table\_\allowbreak five\_\allowbreak structures} & same file\\
...
\eqref{eq:kcmulti} & the instability over several layers & \code{hittingset.\allowbreak rsb\_\allowbreak scale} & \code{figs/\allowbreak hittingset.\allowbreak py}\\
...
\ref{tab:fivehs} & the five structures, hard against soft & \code{figs/\allowbreak hittingset.\allowbreak py:\allowbreak table\_\allowbreak five\_\allowbreak structures} & same file\\
...
\eqref{eq:core} & the LCZP map, three-state leaf removal & \code{core.\allowbreak CorePercolation.\allowbreak solve} & \code{test\_\allowbreak core}\\
\eqref{eq:corethresh} & the core threshold over several layers & \code{core.\allowbreak CorePercolation.\allowbreak core\_\allowbreak free\_\allowbreak spectral} & \code{test\_\allowbreak core}\\
\eqref{eq:sumkappae} & total chy-degree $e$, Poisson layers & \code{core.\allowbreak core\_\allowbreak threshold} & \code{test\_\allowbreak core}\\
...
\eqref{eq:smallcore} & the core at small clique chy-degree & \code{core.\allowbreak CorePercolation.\allowbreak core\_\allowbreak fraction} & \code{figs/\allowbreak cover.\allowbreak py}\\
\eqref{eq:vcinstab} & the cover instability at cardinality $c$ & \code{cover.\allowbreak CliqueCover.\allowbreak solve} & \code{figs/\allowbreak cover.\allowbreak py}\\
\ref{tab:five} & the five structures & \code{figs/\allowbreak cover.\allowbreak py:\allowbreak table\_\allowbreak five\_\allowbreak structures} & same file\\
...
\eqref{eq:vwpi} & the correlated-graph hard-field map & \code{vertexcover.\allowbreak solve} & \code{test\_\allowbreak fixedpoint}\\
\eqref{eq:vwxc} & its cover size, Weigt--Hartmann at $r=0$ & \code{vertexcover.\allowbreak cover\_\allowbreak size} & \code{test\_\allowbreak fixedpoint}\\
\eqref{eq:rc} & where replica symmetry breaks in $r$ & \code{vertexcover.\allowbreak rsb\_\allowbreak point} & \code{test\_\allowbreak fixedpoint}\\
\ref{fig:vw} & the core measured along that axis & \code{statmech/\allowbreak probe/\allowbreak vw\_\allowbreak core.\allowbreak py} & \code{figs/\allowbreak cover.\allowbreak py:\allowbreak figure\_\allowbreak vw}\\
\ref{fig:hrgc} & clustering of the hyperbolic graphs & \code{statmech/\allowbreak probe/\allowbreak hrg\_\allowbreak transitivity.\allowbreak py} & \code{figs/\allowbreak cover.\allowbreak py:\allowbreak figure\_\allowbreak hrg\_\allowbreak c}\\
--- & clustering along the correlation axis & \code{statmech/\allowbreak probe/\allowbreak vw\_\allowbreak clustering.\allowbreak py} & --- (measurement)\\
...
\eqref{eq:colproperline} & the proper line, $(q-1)^{2}$ & \code{figs/\allowbreak colouring.\allowbreak py:\allowbreak check\_\allowbreak cardinality} & same file\\
\eqref{eq:colhyperline} & the hypergraph line, and how far it runs & \code{figs/\allowbreak colouring.\allowbreak py:\allowbreak check\_\allowbreak cardinality} & same file\\
...
\eqref{eq:colmulti} & the stability line over several layers & \code{figs/\allowbreak colouring.\allowbreak py:\allowbreak stability\_\allowbreak threshold\_\allowbreak layers} & same file\\
\ref{tab:fivecol} & the five structures, both rules & \code{figs/\allowbreak colouring.\allowbreak py:\allowbreak table\_\allowbreak five\_\allowbreak structures} & same file\\
...
\eqref{eq:colsigma} & its complexity, against $\kbar_{q}$ & \code{figs/\allowbreak colouring.\allowbreak py:\allowbreak sp\_\allowbreak threshold} & same file\\
...
\eqref{eq:nested} & a clause whose member is a clause & \code{figs/\allowbreak satisfiability.\allowbreak py:\allowbreak nested\allowbreak \_\allowbreak G} & same file\\
\eqref{eq:flatten} & what CNF flattening costs & \code{figs/\allowbreak satisfiability.\allowbreak py:\allowbreak check\allowbreak \_\allowbreak nested\allowbreak \_\allowbreak flatten} & same file\\
...
\eqref{eq:nestedwp} & its warning map & \code{figs/\allowbreak satisfiability.\allowbreak py:\allowbreak nested\allowbreak \_\allowbreak G} & same file\\
...
\section*{One handle, many modules}

Every routine named in this book is reachable two ways, and both are worth
knowing. The modules --- \code{cover}, \code{core}, \code{hittingset} and the
rest --- each solve one problem and are where the algebra is to be read. But a
reader who wants to \emph{run} a result should not have to build a solver by
hand and pass it the right generating function: \code{Chygraph} holds the
structure once, and its methods return the solvers already built from it. So

\begin{quote}
\small\code{g = Chygraph([2, 3], [4.0, 2.0])}\\
\small\code{g.clique\_cover().solve()}
\end{quote}

\noindent
and not \code{CliqueCover(c, phi)} assembled by hand. Table~\ref{tab:map} above names the module against each equation, because that
is the column for finding the source. Table~\ref{tab:handles} is the map from
those modules back to the handle that returns them.

\begin{table}[t]
\centering\small
\begin{tabular}{@{}ll@{}}
\hline\hline
handle on \code{Chygraph} & module it returns\\
\hline
\code{amplitude} & \code{amplitude.\allowbreak CriticalAmplitude}\\
\code{clique\_cover} & \code{cover.\allowbreak CliqueCover}\\
\code{core} & \code{core.\allowbreak CorePercolation}\\
\code{emitted\_field}, \code{u\_prime} & \code{cavity}\\
\code{fixed\_point\_stability} & \code{fixedpoint.\allowbreak FixedPointStability}\\
\code{free\_energy} & \code{freeenergy.\allowbreak BetheFreeEnergy}\\
\code{hitting\_set} & \code{hittingset.\allowbreak HittingSet}\\
\code{hitting\_set\_bp} & \code{softfield.\allowbreak HittingSetBP}\\
\code{population} & \code{population.\allowbreak CavityPopulation}\\
\code{region\_gbp} & \code{gbp.\allowbreak GBP}\\
\code{regions}, \code{overlap\_profile} & \code{region}\\
\code{simplicial} & \code{simplicial}\\
\code{solve\_anti\_monotone} & \code{antimonotone}\\
\code{stability\_tensor} & \code{stability.\allowbreak StabilityMatrix}\\
\hline\hline
\end{tabular}
\caption{\textbf{The handle and what it returns.} \code{Chygraph} holds the
layers, their cardinalities and the chy-degree distribution, and delegates every
calculation rather than reimplementing it, so these return the module's object
already built from the structure. Part~II has the same arrangement one package
over: \code{Chygraph} in \code{percolation.\allowbreak giant} carries
\code{amplitude} and the rest.}
\label{tab:handles}
\end{table}
...
\end{quote}
regenerates Chapter~\ref{ch:ising}'s four figures and reports, among other
...
\code{figs/\allowbreak colouring.\allowbreak py} is the slowest in the book by
a wide margin, and almost all of it is one function: locating the
colourability fold of Sec.~\ref{sec:colouring-cq} at the population where it
has converged costs about eighty minutes, against fourteen for everything else
in the file put together. That is the price of the third decimal in
Table~\ref{tab:cq}. It is bisection over three seeds and two values of $q$ at
$60\,000$ samples, and it buys little at the last digit: a fifth of the
population returns $3.074$ where the converged run returns $3.073$, and the
conclusion the number is used for --- an inequality --- is indifferent to
either. A reader who wants the file to finish quickly should pass a smaller
\code{size} to \code{check\_\allowbreak clustered\_\allowbreak cq} and
expect the last digit to move.
\code{figs/\allowbreak satisfiability.\allowbreak py} takes about five minutes,
also for its survey propagation;
