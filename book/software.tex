...
That last part is the reason the chapter exists. A book of this kind accumulates
a great many closed forms, and a reader who wants to use one of them is entitled
to ask what it was checked against. Table~\ref{tab:map} answers that for every
numbered equation the book actually computes with.

\section*{Where it is}

Two packages, both installable and both with their own test suite:

\begin{quote}
\small\url{https://github.com/av2atgh/chygraph}
\end{quote}
\noindent
carries percolation on chygraphs: the self-consistency map, the threshold
tensor, the critical amplitude, correlated layers, and the constructions of
Chapters~\ref{ch:percolation}--\ref{ch:epidemics}. It is symbolic throughout,
on \code{sympy}.

\begin{quote}
\small\url{https://github.com/av2atgh/chygraph_statmech}
\end{quote}
\noindent
carries everything from Chapter~\ref{ch:statmech} onward: the cavity recursion
with a general interior, the branching matrix, the Ising and hitting-set and
core-percolation solvers, the Bethe free energy, and the region-graph
calculation of Chapter~\ref{ch:overlap}.

A third source is the book's own \code{figs/} directory, which holds one script
per chapter. Those scripts generate the figures, and for
Chapters~\ref{ch:potts}, \ref{ch:colouring} and~\ref{ch:satisfiability} they
also carry the calculations themselves, those three chapters having no
manuscript behind them.

Alongside the packages sit two directories worth naming, because several
results in the book are read from them rather than recomputed. \code{examples/}
...
comparison of Sec.~\ref{sec:soft}. \code{probe/} holds the expensive
measurements, with their outputs cached under \code{probe/results/}: the Monte
Carlo of Eq.~\eqref{eq:mc}, the leaf-removal validation of
Sec.~\ref{sec:cover-checks}, the hyperbolic-graph scan behind
Fig.~\ref{fig:hrg}, and the sixty region graphs behind Fig.~\ref{fig:gbp}.
Those four are the only places where the book quotes a number it does not
recompute at build time, and each says so where it is used.
...
core-free branch. Two chapters' worth of argument, enforced in one predicate.
...
Sec.~\ref{sec:unanimity} shows. A function that cannot be used incorrectly is
worth more than a note in its documentation.
...
\code{chygraph} for Part~II and \code{chygraph\_statmech} for Part~III unless a
path is given. Tests are the packages' own suites; \code{figs/} entries are the
...
\eqref{eq:contagion} & threshold contagion & \code{figs/\allowbreak epidemics.\allowbreak py:\allowbreak branch} & same file\\
...
\eqref{eq:mc} & Monte Carlo $T_{c}$ & \code{probe/\allowbreak ising\_\allowbreak mc.\allowbreak py} & --- (simulation)\\
...
--- & generalised belief propagation & \code{gbp.\allowbreak GBP} & \code{test\_\allowbreak gbp}\\
\end{longtable}
}
...
Every figure in the book is generated by a script in \code{figs/\allowbreak }, one per
chapter, named for it. Running a script rewrites its PDFs into the same
...
Two of the scripts are slow, and for the same reason: they solve coexistence
windows by bisection. \code{figs/\allowbreak ising.\allowbreak py} and \code{figs/\allowbreak potts.\allowbreak py} take a
minute or two each. The rest run in seconds.
