...
\begin{equation}
  ...
  \label{eq:vc}
\end{equation}
Equation~\eqref{eq:vcz} is a statement about one inclusion of one graph and
Eq.~\eqref{eq:vc} is the same statement averaged over an ensemble, and it is
worth saying how the second follows from the first, because it does not follow
by inspection.

\emph{What the message says.} $z_{i\to a}=1$ means $i$ is \emph{free} --- it may
be left out of the cover --- as seen by $a$. The sum counts the complexes other
than $a$ that block it, and $\max(0,\max_{j\in b\setminus i}z_{j\to b})$ is $1$
exactly when at least one other member of $b$ is itself free. That is the
covering constraint read locally: two members of a complex are adjacent, so if
$j$ is left out then $i$ must be taken. Writing the inner cost as a maximum
rather than a sum is what makes blocking a yes-or-no event per complex instead
of a count, and a yes-or-no event per complex is what can be pushed through a
generating function.

\emph{What a complex does.} Let $\sigma_{l}$ be the probability that a node
reached along an inclusion into a layer-$l$ complex is free. Entered at $i$,
such a complex leaves $i$ free only if none of its other $c_{l}-1$ members is
free, and on a treelike chygraph those members are independent given what
reaches them, so $\tau_{l}=(1-\sigma_{l})^{\,c_{l}-1}$.

\emph{What a node does.} $i$ is free when \emph{every} one of its other
complexes leaves it free. How many others there are is the excess chy-degree of
Sec.~\ref{sec:ensembles}, and averaging a product over that count is evaluating
its generating function at $\tau$, which gives $\sigma_{m}=\bar\Phi^{(m)}(\tau)$
and so Eq.~\eqref{eq:vc}. Section~\ref{sec:treelike}'s condition is used twice
...
share nothing. The interior factor $\tau$ is
\code{cover.\allowbreak CliqueCover.\allowbreak tau} and the map is \code{cover.\allowbreak CliqueCover.\allowbreak solve},
which reaches its fixed point by the bracket of Sec.~\ref{sec:hard} rather than
by plain iteration, both maps being order-reversing.
...
node entering it. Enter a complex through one member and look at the other
$c-1$. With $m$ of them still live, every live member has clique-degree exactly
$m$ inside the complex, so two events matter and no others.
...
ways to choose which member is the live one, so
$\xi=(c-1)\,\delta^{\,c-2}\psi$.

At $c=2$ these degenerate to $\zeta=\delta$ and $\xi=\psi$, returning the graph
recursion $\psi=\bar\Phi(\delta)$, $\delta=1-\bar\Phi(1-\psi)$. The whole of it
--- the three states, the two intra-complex quantities and the fixed point --- is
\code{core.\allowbreak CorePercolation.\allowbreak solve}, which iterates monotonically from zero
because, unlike Eq.~\eqref{eq:vc}, this map is order-preserving.
...
and $f(1)=0$. The only root is $\delta=1$, forcing $\psi=0$. The predicate is
\code{core.\allowbreak CorePercolation.\allowbreak has\_\allowbreak core\_\allowbreak free\_\allowbreak branch}, and it returns true exactly
when every cardinality is two.
...
The three columns are Sec.~\ref{sec:hrg}'s. \emph{Measured} is leaf removal run
on the graph. \emph{Chygraph} is Eq.~\eqref{eq:core} evaluated on the
maximal-clique ensemble measured from that same graph, with nothing fitted.
\emph{Control} is leaf removal on the degree-matched configuration model, which
keeps $p(k)$ and destroys the clustering, averaged over twenty rewirings.

\begin{table}[t]
\centering\small
\input{figs/tab-real-core}
\caption{\textbf{Leaf removal on sixteen real networks.} Core fraction
$P_{C}(\mathrm{VC},\mathrm{BP}_{\chi})$ measured on the graph, predicted by
Eq.~\eqref{eq:core} on the graph's own maximal-clique ensemble, and measured on
the degree-matched control. Ordered by mean degree, which is what decides
whether the comparison can say anything. Generated by
