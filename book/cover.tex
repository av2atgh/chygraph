\chapter{Vertex cover and core percolation}
\label{ch:cover}

Three things in this chapter turn out to be one thing, and saying which three
is most of the work.

There is an \emph{algorithm}, leaf removal, which sometimes solves minimum
vertex cover exactly and sometimes gets stuck. There is a \emph{structure}, the
core, which is what it gets stuck on --- and which has a percolation transition
of its own, at mean degree $e$. And there is the \emph{instability} of
Chapter~\ref{ch:hittingset}'s hard-field map, Eq.~\eqref{eq:kc}, which also sits
at $e$. These are not three results that happen to share a number. They are one
point computed three ways, and Sec.~\ref{sec:onepoint} says so with the digits.

Then the chapter does what Chapter~\ref{ch:hittingset} did: raises the
cardinality and watches something break. Above cardinality two the core-free
...
is back to being hard. So core percolation is not a fifth model in this book. It
is the obstruction to solving the fourth, and that is why it belongs in the same
chapter.
...
The three-state cavity treatment of core percolation on graphs is
Ref.~\citep{liu2012}; what follows carries it to complexes.

Writing $(\lambda,\delta,\gamma)$ for their probabilities, the chy-degree step is
\begin{equation}
  \lambda_{m}=\bar\Phi^{(m)}(\zeta),
  \qquad
  \delta_{m}=1-\bar\Phi^{(m)}(1-\kappa),
  \label{eq:core}
\end{equation}
...
At $c=2$ these degenerate to $\zeta=\delta$ and $\kappa=\lambda$, returning the
graph recursion $\lambda=\bar\Phi(\delta)$,
$\delta=1-\bar\Phi(1-\lambda)$.
\end{calculation}
...
At $c=2$, Eq.~\eqref{eq:core} has threshold $\bar\Phi'(1-\lambda)=1$, which for
...
$2.5\times10^{-10}$ --- which is not a coincidence to be admired but a
consistency check to be passed, since they are the same point reached by two
routes.

The identification is worth stating in words, because it is the reason this
book puts the three subjects in one chapter. \emph{Leaf removal certifies a
...
Now raise the cardinality, and something abrupt happens.

\begin{calculation}{no core-free branch above cardinality two}
A core-free branch means $\gamma=0$, that is $\lambda+\delta=1$. Substituting
Eq.~\eqref{eq:core},
\[
  \bar\Phi^{(m)}(\zeta)+1-\bar\Phi^{(m)}(1-\kappa)=1,
\]
and since $\bar\Phi$ is a power series with non-negative coefficients, hence
strictly increasing, this requires $\zeta=1-\kappa$ in every layer. With
...
and $f(1)=0$. The only root is $\delta=1$, forcing $\lambda=0$.
\end{calculation}
...
past three changes nothing, which is the sort of degeneracy worth noticing: the
obstruction is not a matter of degree.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{figs/fig-core.pdf}
\caption{\textbf{The core-free branch, and its disappearance.} Leaf-removal core
against chy-degree for pure clique networks. At cardinality two there is a
...
\paragraph{The cleanest control, and why it is the cleanest.} There is one place
...
\paragraph{How much this is worth, stated plainly.} Less than it looks, and the
book had better say so. Section~\ref{sec:corefree} shows that \emph{any} layer of
...
exponent does not move with $\tau$ at all --- $1.55$, $1.57$, $1.57$ across
$\tau=2.5$, $2.9$, $2.1$ --- while the measured one spreads by seven times as
much and rises monotonically, $1.59$, $1.64$, $1.49$. The two agree within
errors at $\tau=2.5$ and differ at $\tau=2.9$ by roughly four standard
...
\emph{Against known results.} The $c=2$ threshold returns $e$ to ten digits,
which is Bauer--Golinelli. Equation~\eqref{eq:vc} at $c=2$ returns the
Weigt--Hartmann cover size, agreeing with Eq.~\eqref{eq:hs} to nine digits along
a different code path.

\emph{Against arithmetic.} Equation~\eqref{eq:factored} is an identity, checked
symbolically; the numerical map agrees, reporting a core-free branch at $c=2$
and none at $c=3,4,5,8$. That the core is exactly $1-e^{-\ave{k}}$ above
cardinality two is checked against the closed form at three cardinalities and
three densities.

\emph{Against simulation.} Equation~\eqref{eq:core} is compared with pure leaf
removal on graphs built to match it, at $n=4\times10^{5}$, over eleven
ensembles at cardinalities two, three and four. The error runs from
$8.7\times10^{-5}$ to $3.1\times10^{-3}$.

\emph{Against a network nobody designed.} Section~\ref{sec:hrg}. Two
qualifications carried with it: the existence of the core follows from the
algebra rather than from the data, and the exponent comparison is weaker than
its central values suggest. Both are stated there rather than in a footnote,
because a reader who takes away only the figure will have taken away too much.
