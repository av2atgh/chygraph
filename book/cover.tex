...
\begin{equation}
  ...
  \label{eq:vc}
\end{equation}
At $c=2$ this collapses onto Eq.~\eqref{eq:hs}, as it must --- both return the
Weigt--Hartmann cover size, to nine digits, along code paths that share nothing.

\begin{calculation}{from the message to the map}
Equation~\eqref{eq:vcz} is a statement about one inclusion of one graph;
Eq.~\eqref{eq:vc} is the same statement averaged over an ensemble. Two readings
and one average take you from the first to the second.
...
its generating function at $\tau$:
\[
  \sigma_{m}=\bar\Phi^{(m)}(\tau),
\]
which is Eq.~\eqref{eq:vc}. Sec.~\ref{sec:treelike}'s condition is used twice
over, in two different places: once inside a complex, to multiply the $c_{l}-1$
members, and once across the complexes at a node, to evaluate $\bar\Phi$. It is
the first of the three assumptions Sec.~\ref{sec:assumes} lists, and it is
carrying both steps here.
...
over $\ave{k}\le1$. Data from \code{statmech/\allowbreak probe/prediction4.py}; plotted by
\code{figs/cover.py}.}
\label{fig:hrg}
\end{figure}
