...
A museum wants a guard standing at some of its junctions, positioned so that
every corridor is watched from at least one end, and it wants to hire as few
guards as it can. Nothing about the question is subtle, and no method is known
that answers it quickly in general. What makes it a problem for statistical
physics is that on a random graph it is not uniformly hard. Below a certain
density a purely local rule --- look for a corridor with a dead end, put the
guard at the other end of it, and repeat --- finds the smallest possible set of
guards and proves that it is smallest. Above that density the rule runs out of
dead ends and stops, with most of the museum still unwatched. The density at
which it stops is a phase transition, and this chapter locates it three separate
ways and then asks what happens when the corridors arrive in bunches.
...
a hard constraint forbidding a link with neither endpoint taken,
Eq.~\eqref{eq:vc-intro} --- and it is NP-hard
...
\section{Vertex covering leaf removal}
...
\label{sec:vw}
\index{degree correlations|textbf}
\index{assortativity}
\index{replica symmetry breaking}
\index{transitivity}
\index{clustering!manufactured by degree correlations}
...
and $0.80$ at $\tau=3.0$, against $0.70$ and $0.7802$ from
...
$r_{\mathrm{c}}$ was $0.70$ against $0.7802$, a wider spread and not a
