...
\paragraph{A graph} is one Poisson layer of links at chy-degree two, and is the
row the others are read against. All three methods
...
\paragraph{A hypergraph} is one Poisson layer of $3$-cliques at chy-degree one,
the same two neighbours per node as the row above. It loses all of the above at
once, and the loss is structural rather than a matter of density. Note first which problem is being solved: this
...
each. It is on the good side and stays there however many link types it has,
every layer being a link layer at $c=2$. What the extra
...
layers at chy-degree $2/3$ for the graph reading, three layers of $3$-cliques at
chy-degree $1/3$ for the hypergraph one. It is the panel that can land on either
side, and the layer list decides which. As Fig.~\ref{fig:mappings} draws
...
\paragraph{A clustered graph} is a Poisson link layer at chy-degree one beside a
Poisson triangle layer at chy-degree one half. It is formally on the bad side
and is the interesting one. It is the mapping Part~III rests on, and the mapping
