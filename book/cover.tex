...
it belongs in the same chapter.
...
\caption{\textbf{Leaf removal on sixteen real networks.} The rows of
Table~\ref{tab:realcore} drawn. Left: core fraction against mean degree.
Zeros sit on the axis floor, as in Fig.~\ref{fig:hrg}; the dotted line is
$\ave{k}=e$, where a treelike graph acquires a core. Below it the
degree-matched control has none and the network and its chygraph do; above it
all three saturate and the comparison distinguishes nothing. Right: prediction
against measurement, with the diagonal. The five filled points are the ones the
comparison rests on --- the networks whose control has no core --- and they lie
below the diagonal, the chygraph understating by the factors quoted in the text.
The four at the left edge have no measured core, and the chygraph puts them
between $2\times10^{-4}$ and $10^{-3}$. Generated by
\code{figs/cover.py:figure\_real\_core}.}
\label{fig:realcore}
\end{figure}

Figure~\ref{fig:realcore} draws the same numbers, and the shape of the table is
easier to read there than in the digits.
