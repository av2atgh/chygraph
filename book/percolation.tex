...
Percolation on a chygraph was solved in
Refs.~\citep{vazquez2023pre,vazquez2024comnet}, and this chapter is the account
of it. It is placed first among the applications for a reason that will become
clear only in Chapter~\ref{ch:statmech}: percolation is the case in which the
message travelling along an inclusion is a single number. Everything harder in
this book is the same recursion carrying something heavier.

The chapter has one main claim, and it is a claim about labour rather than about
physics. The higher-order network literature has produced a separate percolation
...
Section~\ref{sec:helporhurt} is not in the papers. It settles a question the
chapter raises against itself.

\section{What percolation on a chygraph asks}
\label{sec:whatasks}
...
That last piece of notation carries a real idea and is worth unpacking. Arriving
...
Thinning is not always the right move, and the exception is worth flagging now
because Chapter~\ref{ch:giant} turns on it. When the connectivity rule already
...
$2L^{2}$ unknowns, and the transition is where its largest eigenvalue passes
through zero. That operator is the tensor $A$ of Ref.~\citep{vazquez2023pre}.

\begin{calculation}{the threshold tensor}
...
\[
  ...
  \end{bmatrix},
\]
and the order parameter is its largest eigenvalue,
\[
  \Lambda=\max_{\lambda}\ \mathrm{eigenvals}\bigl(-\mathrm{vec}^{2}A\bigr).
\]
Finite components when $\Lambda<0$, a giant component when $\Lambda>0$.
...
than $\Lambda$. One warning, and it is a real one: $\theta$ vanishes where
...
$\theta$ to locate the transition and $\Lambda$ to say which side of it you are
on.
\end{calculation}

Two features of this are worth naming before the examples, because they are what
make the construction worth having.
...
matrices for the structure of Sec.~\ref{sec:everything} and substituting. The
monotony is the point.
...
\begin{equation}
  ...
  \label{eq:thetaG}
\end{equation}
For $p=q=1$ this is Molloy and Reed \citep{molloy1995}; for general $p$ and $q$
it is Callaway \emph{et al.} \citep{callaway2000}. It is also
Eq.~\eqref{eq:molloyreed}, which is the reassurance one wants at this point:
the machinery reproduces Chapter~\ref{ch:introduction} exactly.
...
\begin{equation}
  ...
  \label{eq:thetaMP}
\end{equation}
recovering Sun and Bianconi \citep{sun2021}. The contribution of the layers is
...
Ref.~\citep{vazquez2024comnet}. The matrices are sparse and the substitution is
mechanical. Nothing further is required, which is the point: interacting
...
is genuinely different, and it is the one the second half of the book depends
on. Give a graph a layer of plain links ($c=2$) and a layer of triangles
...
\begin{calculation}{what a bond-percolated triangle transmits}
...
Equation~\eqref{eq:thetaNM} invites an obvious question, and the published
answer to it \citep{vazquez2024comnet} sits awkwardly beside a result this book
reaches later. It is worth resolving the tension rather than letting the reader
find it.
...
infection more easily too. This agrees with Newman \citep{newman2003}.
...
broaden. Generated by \code{figs/triangles\_vs\_regular.py}.}
\label{fig:triangles}
\end{figure}

The mechanism is the one Chapter~\ref{ch:ising} will give for the Ising model,
and it is worth stating in the form that covers both. A triangle transmits
...
transmission of a magnetisation, is the whole of Chapter~\ref{ch:ising}'s
headline result.

\section{Checks}
\label{sec:percolation-checks}

Three kinds of check run through this chapter and the next, and they answer
different questions.

\emph{Against known results.} Every specialisation in
Sec.~\ref{sec:structurebystructure} reproduces a published formula ---
Molloy--Reed, Callaway \emph{et al.}, the hypergraph result, Sun and Bianconi's
multiplex expression. This tests the algebra and nothing else.

\emph{Against Monte Carlo on a random chygraph.} Sample a chygraph from the
configuration-model construction of Sec.~\ref{sec:sampling}, percolate it, and
compare. This tests the map. It does not test the assumption the map rests on,
because a chygraph built that way has complexes meeting in at most one atom by
construction --- as Sec.~\ref{sec:sampling} warned.

\emph{Against Monte Carlo on a structure built to have overlap.}
Figure~\ref{fig:triangles} is halfway to this: the two-triangles-per-vertex
network is a real graph, constructed with the triangles sharing at most one node
so that the theory should be exact, and it is. Building one where they share
more is Chapter~\ref{ch:overlap}'s business, and there the theory is not exact.

Nothing in this chapter needed the complexes to be small, or the layers to be
few, or the cardinality distribution to be narrow. What it needed was that the
complexes meet in at most one atom. That single assumption is what
Chapter~\ref{ch:overlap} has to pay for, and everything between here and there
is what it buys.
