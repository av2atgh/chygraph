...
Pour water onto a bed of sand and it either drains through or it does not.
Remove enough bonds from a conducting lattice and the material stops conducting,
not gradually but at a stroke. Immunise enough of a population and an outbreak
that was inevitable becomes impossible. In each of these a system built out of
many small local connections has one global property --- whether something can
get from here to there --- and that property does not fade as the connections
are taken away. It survives, and survives, and then at one density it is gone.

That density is the subject of percolation theory, and the transition at it is
the simplest phase transition there is. There is no temperature in it, no
energy and no Hamiltonian; there is nothing but connectivity. Which is why it
comes first here. Everything else in this book is a harder question asked of the
same recursion, and the recursion is easiest to see when almost nothing is
riding on it.
...
for simplicial complexes and for clustered graphs, and the number of those
derivations keeps growing
...
They hurt, once the comparison is made properly. At a fixed degree distribution,
putting a node's links into triangles rather than spending them singly raises the
percolation threshold by $20.9$ per cent, which is Eq.~\eqref{eq:qc} below. At a
fixed \emph{link count} it lowers the threshold instead, which is
Eq.~\eqref{eq:deltatheta}, and that is the published answer
\citep{vazquez2024comnet}. Both calculations are right. They are not the same
comparison, and the second one is not measuring clustering at all.

Equation~\eqref{eq:thetaNM} raises the question, and the published answer
appears to conflict with a result this book reaches later. This section
resolves the conflict, and the resolution is a caution about control networks
rather than a correction to either calculation.
